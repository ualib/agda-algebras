...
open import Overture.Setoid.Inverses           using ( Img_∋_ ; eq )
open import Overture.Setoid.Surjective         using ( isSurj ; IsSurjective ; isSurj→IsSurjective )
open import Algebras.Setoid.Basic      {𝑆 = 𝑆} using ( Algebra ; 𝕌[_] ; _̂_ )
open import Homomorphisms.Setoid.Basic {𝑆 = 𝑆} using ( hom ; compatible-map ; IsHom )
open import Terms.Basic              {𝑆 = 𝑆} using ( Term )
open import Terms.Setoid.Basic         {𝑆 = 𝑆} using ( 𝑻 ; _≐_  ; ≐-isRefl )
open Term
...
<span style="float:right;">[Terms.Setoid.Operations →](Terms.Setoid.Operations.html)</span>
