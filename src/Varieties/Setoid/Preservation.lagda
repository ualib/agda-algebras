---
layout: default
title : "Varieties.Setoid.Preservation (The Agda Universal Algebra Library)"
date : "2021-09-24"
...
This is the [Varieties.Setoid.Preservation][] module of the [Agda Universal Algebra Library][] where we show
...
module Varieties.Setoid.Preservation {𝑆 : Signature 𝓞 𝓥} where
...
open import Overture.Setoid.Surjective                using ( IsSurjective ; SurjInv ; SurjInvIsInverseʳ )
open import Algebras.Setoid.Basic             {𝑆 = 𝑆} using ( Algebra ; ov ; 𝕌[_] ; Lift-Alg )
open import Algebras.Setoid.Products          {𝑆 = 𝑆} using ( ⨅ )
open import Homomorphisms.Setoid.Basic        {𝑆 = 𝑆} using ( hom )
open import Homomorphisms.Setoid.Isomorphisms {𝑆 = 𝑆} using ( ≅⨅⁺-refl ; ≅-refl ; ≅-sym ; _≅_ ; ≅-trans ; Lift-≅ )
open import Homomorphisms.Setoid.HomomorphicImages {𝑆 = 𝑆} using ( IdHomImage )
open import Terms.Basic                     {𝑆 = 𝑆} using ( Term )
open import Terms.Setoid.Basic                {𝑆 = 𝑆} using ( module Environment)
open import Terms.Setoid.Operations           {𝑆 = 𝑆} using ( comm-hom-term )
open import Subalgebras.Setoid.Subalgebras    {𝑆 = 𝑆} using ( _≤_ ; _≤c_ )
open import Subalgebras.Setoid.Properties     {𝑆 = 𝑆} using ( ⨅-≤ ; ≅-trans-≤ ; ≤-reflexive )
open import Varieties.Setoid.Closure {𝑆 = 𝑆} using ( H ; S ; P ; V ; S-expa ; H-expa ; P-expa
                                                 ; V-expa ; Level-closure )
open import Varieties.Setoid.Properties {𝑆 = 𝑆} using ( ⊧-H-invar ; ⊧-S-invar ; ⊧-P-invar ; ⊧-I-invar )
open import Varieties.Setoid.SoundAndComplete  {𝑆 = 𝑆} using ( _⊧_ ; _⊨_ ; _⊫_ ; Eq ; _≈̇_ ; lhs ; rhs ; _⊢_▹_≈_ ; Th)
...
<span style="float:left;">[← Varieties.Setoid.Properties](Varieties.Setoid.Properties.html)</span>
