---
layout: default
title : "Varieties.Setoid.Properties module (The Agda Universal Algebra Library)"
date : "2021-09-24"
...
module Varieties.Setoid.Properties {𝑆 : Signature 𝓞 𝓥} where
...
open import Overture.Setoid.Inverses                        using ( InvIsInverseʳ )
open import Overture.Setoid.Surjective                      using ( SurjInv )
open import Algebras.Setoid.Basic                   {𝑆 = 𝑆} using ( Algebra ; Lift-Algˡ ; ov ; 𝕌[_] ; 𝔻[_] )
open import Algebras.Setoid.Products                {𝑆 = 𝑆} using ( ⨅ )
open import Homomorphisms.Setoid.Basic              {𝑆 = 𝑆} using ( hom )
open import Homomorphisms.Setoid.Isomorphisms       {𝑆 = 𝑆} using ( _≅_ ; mkiso ; Lift-≅ˡ ; ≅-sym )
open import Homomorphisms.Setoid.HomomorphicImages  {𝑆 = 𝑆} using ( _IsHomImageOf_ )
open import Terms.Basic                             {𝑆 = 𝑆} using ( Term ; ℊ )
open import Terms.Setoid.Basic                      {𝑆 = 𝑆} using ( 𝑻 ; module Environment )
open import Terms.Setoid.Operations                 {𝑆 = 𝑆} using ( comm-hom-term ; interp-prod )
                                                            using ( term-agreement )
open import Subalgebras.Setoid.Subalgebras          {𝑆 = 𝑆} using ( _≤_ ; SubalgebrasOfClass )
open import Varieties.Setoid.SoundAndComplete       {𝑆 = 𝑆} using ( _⊧_ ; _⊨_ ; _⊫_ ; Eq ; _≈̇_ )
...
<span style="float:right;">[Varieties.Setoid.Preservation →](Varieties.Setoid.Preservation.html)</span>
