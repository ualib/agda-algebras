---
layout: default
title : "Varieties.Setoid.HSP module (Agda Universal Algebra Library)"
date : "2021-10-16"
...
module Varieties.Setoid.HSP {𝑆 : Signature 𝓞 𝓥} where
...
open import Relations.Setoid.Discrete                       using ( fkerPred )
open import Algebras.Setoid.Basic                   {𝑆 = 𝑆} using ( Algebra ; ov ; Lift-Alg )
open import Algebras.Setoid.Products                {𝑆 = 𝑆} using ( ⨅ )
open import Homomorphisms.Setoid.Basic              {𝑆 = 𝑆} using ( hom ; mon ; IsMon ; IsHom )
                                                            using ( epi ; epi→ontohom )
open import Homomorphisms.Setoid.Products           {𝑆 = 𝑆} using ( ⨅-hom-co )
open import Homomorphisms.Setoid.Factor             {𝑆 = 𝑆} using ( HomFactor )
open import Homomorphisms.Setoid.Isomorphisms       {𝑆 = 𝑆} using ( ≅-refl )
open import Homomorphisms.Setoid.HomomorphicImages  {𝑆 = 𝑆} using ( _IsHomImageOf_ )
open import Subalgebras.Setoid.Subalgebras          {𝑆 = 𝑆} using ( _≤_ ; mon→≤ )
open import Terms.Setoid.Basic                      {𝑆 = 𝑆} using ( module Environment ; 𝑻 )
open import Terms.Setoid.Properties                 {𝑆 = 𝑆} using ( lift-hom ; free-lift )
open import Terms.Setoid.Operations                 {𝑆 = 𝑆} using ( free-lift-interp )
open import Varieties.Setoid.SoundAndComplete       {𝑆 = 𝑆} using ( module FreeAlgebra ; _⊫_ ; _≈̇_ )
                                                            using ( _⊢_▹_≈_ ; Mod ; Th )
open import Varieties.Setoid.Closure                {𝑆 = 𝑆} using ( S ; V ; P ; S-idem ; V-≅-lc )
open import Varieties.Setoid.Preservation           {𝑆 = 𝑆} using ( S-id2 ; PS⊆SP )
open import Varieties.Setoid.FreeAlgebras           {𝑆 = 𝑆} using ( module FreeHom ; 𝔽-ModTh-epi-lift )
...
proved that `V` is expansive in the [Varieties.Setoid.Closure][] module:
