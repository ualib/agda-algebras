...
module _  {X : Type χ} where
 ...
 _⊧_≈_ : SetoidAlgebra α ρᵃ → Term X → Term X → Type _
 𝑨 ⊧ p ≈ q = ∀ (ρ : ∣ Env X ∣) → ⟦ p ⟧ ⟨$⟩ ρ ≈ ⟦ q ⟧ ⟨$⟩ ρ
  where
  open Setoid ( Domain 𝑨 ) using ( _≈_ )
  open Environment 𝑨 using ( Env ; ⟦_⟧ )

 _⊫_≈_ : Pred(SetoidAlgebra α ρᵃ) ℓ → Term X → Term X → Type (χ ⊔ ℓ ⊔ ov(α ⊔ ρᵃ))
 𝒦 ⊫ p ≈ q = {𝑨 : SetoidAlgebra _ _} → 𝒦 𝑨 → 𝑨 ⊧ p ≈ q
...
\begin{code}
 ...
 Th : Pred (SetoidAlgebra α ρᵃ) ℓ → Pred(Term X × Term X) (χ ⊔ ℓ ⊔ ov(α ⊔ ρᵃ))
 Th 𝒦 = λ (p , q) → 𝒦 ⊫ p ≈ q

Th' : {χ α : Level}{X : Type χ} → Pred (SetoidAlgebra α α) (ov α)
...
module _ {X : Type χ}{𝒦 : Pred (SetoidAlgebra α ρᵃ) (ov α)} where
...
\begin{code}
 ...
 Mod : Pred(Term X × Term X) (ov α) → Pred(SetoidAlgebra α ρᵃ) (ρᵃ ⊔ ov(α ⊔ χ))
...
\begin{code}
 ...
 Modᵗ : {I : Type ι} → (I → Term X × Term X) → {α : Level} → Pred(SetoidAlgebra α ρᵃ) (χ ⊔ ρᵃ ⊔ ι ⊔ α)
