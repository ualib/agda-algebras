...
substantive theorems — most importantly the Jeavons Galois connection itself for a
fixed finite domain, and a statement of the Bulatov–Zhuk algebraic dichotomy — are
...
We associate with 𝒜 a *constraint satisfaction problem* denoted by CSP(𝒜), which is
...
then we say that CSP(𝒜) is *tractable*.  Otherwise, CSP(𝒜) is *intractable*.
...
CSP(𝒜) of 𝑅 structures having homomorphisms into 𝒜.  That is, our algorithm must take
...
If R' ⊆ (∣: ⃖ R) ⃗ ∣: is finite, then CSP((A, R')) is reducible in poly-time to CSP(𝒜)
...
lives at the same relation level (in practice `0ℓ`).  Lifting `ρʳ` to a
...
subalgebras of the product ⨅ var 𝒜.
...
relation over (some of) the members of 𝒜.
