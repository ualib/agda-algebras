...
open import Relation.Binary                using ( Setoid )
                                           renaming  ( Rel to BinaryRelation
                                                     ; _⇒_ to _⊆_ )
...
##### A hom kernel that collapses `R` contains the congruence generated by `R`
...
module _ {𝑨 : Algebra {𝑆 = 𝑆} α ρᵃ}{ℓ r : Level} where
  ...
  ToFromLift = lift ≈refl


  ToLift-epi : epi 𝑨 (Lift-Alg 𝑨 ℓ r)
  ToLift-epi .proj₁ = ToLift .proj₁
  ToLift-epi .proj₂ .IsEpi.isHom = ToLift .proj₂
  ToLift-epi .proj₂ .IsEpi.isSurjective = eq (FromLift .proj₁ ⟨$⟩ _) ToFromLift
```
...
module _ {𝑨 : Algebra {𝑆 = 𝑆} α ρᵃ} {𝑩 : Algebra β ρᵇ} where
  open Setoid 𝔻[ 𝑨 ]  using ( reflexive )  renaming ( _≈_ to _≈₁_ )
  open Level using (lift ; lower)
  ...
  Lift-homʳ (f , fhom) rᵃ rᵇ = ϕ , Goal
    ...
    ϕ : 𝔻[ lA ] ⟶ 𝔻[ lB ]
    ϕ ⟨$⟩ x = f ⟨$⟩ x
    ϕ .cong xy .lower = f .cong (lower xy)

    Goal : IsHom lA lB ϕ
    Goal = ⊙-is-hom lABh (ToLiftʳ .proj₂)

  open Setoid using ( _≈_ ) renaming (refl to ≈refl)

  lift-hom-lemma : (h : hom 𝑨 𝑩)(a : 𝕌[ 𝑨 ])(ℓᵃ ℓᵇ : Level)
    → 𝔻[ Lift-Algˡ 𝑩 ℓᵇ ] ._≈_ (lift (h .proj₁ ⟨$⟩ a)) (Lift-homˡ h ℓᵃ ℓᵇ .proj₁ ⟨$⟩ lift a)
  lift-hom-lemma h a ℓᵃ ℓᵇ = ≈refl 𝔻[ 𝑩 ]
