...
+  [Setoid.Algebras.Basic][] for the `Algebra`{.AgdaRecord} record, the smart constructors `mkAlgebra`{.AgdaFunction} and `mkAlgebraₚ`{.AgdaFunction}, the interpretation operator `_^_`{.AgdaFunction}, the domain and carrier projections `𝔻[_]`{.AgdaFunction} and `𝕌[_]`{.AgdaFunction}, and universe lifting;
+  [Setoid.Algebras.Products][] for products of indexed families and of classes;
+  [Setoid.Algebras.Finite][] for the `FiniteAlgebra`{.AgdaRecord} interface;
+  [Setoid.Algebras.Reduct][] for reducts along a signature morphism, and their functoriality.
