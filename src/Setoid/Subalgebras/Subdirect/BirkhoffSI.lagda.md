...
```agda
SubdirectSIRep : {𝑆 : Signature 𝓞 𝓥}(𝑨 : Algebra {𝑆 = 𝑆} α ρ) → (ℓ ι : Level) → Type (𝓞 ⊔ 𝓥 ⊔ α ⊔ ρ ⊔ lsuc (ℓ ⊔ ι))
...
```agda
SIRep→Representable : {𝑨 : Algebra {𝑆 = 𝑆} α ρ}
...
```agda
module _ (sirep : (𝑨 : Algebra {𝑆 = 𝑆} α ρ) → SubdirectSIRep 𝑨 ℓ ι) where
  Birkhoff-subdirect : (𝑨 : Algebra {𝑆 = 𝑆} α ρ) → SubdirectlyRepresentable 𝑨 ℓ ι
