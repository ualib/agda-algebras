---
layout: default
title : "Setoid.Algebras.Congruences.CompleteLattice module (The Agda Universal Algebra Library)"
date : "2026-06-02"
...
This is the [Setoid.Algebras.Congruences.CompleteLattice][] module of the [Agda Universal Algebra Library][].

[Setoid.Algebras.Congruences.Lattice][] gave the meet (intersection) and the
containment order, and [Setoid.Algebras.Congruences.Generation][] gave the join
`θ ∨ φ = Cg(θ ∪ φ)` via the congruence-generation theorem.  This module assembles
those pieces into the **congruence lattice** of a setoid algebra and shows it is
*complete*.
...
> `L = 𝓞 ⊔ 𝓥 ⊔ α ⊔ ρ ⊔ ℓ₀`  (for a base level `ℓ₀`), at which `𝒈 L = L`.
...
of [Setoid.Algebras.Lattices.CompleteLattice][].
...
module Setoid.Algebras.Congruences.CompleteLattice {𝑆 : Signature 𝓞 𝓥} where
...
open import Setoid.Algebras.Congruences.Generation  {𝑆 = 𝑆}
...
```agda
 Con-supremum : Supremum (_≤_ {𝑨 = 𝑨} {L}) (_∨_ {𝑨 = 𝑨} {L})
 Con-supremum θ φ = ∨-upperˡ {𝑨 = 𝑨} {θ = θ} {φ}
                  , ∨-upperʳ {𝑨 = 𝑨} {θ = θ} {φ}
                  , λ ψ θ≤ψ φ≤ψ → ∨-least {𝑨 = 𝑨} {θ = θ} {φ} ψ θ≤ψ φ≤ψ
```
...
```agda
 ...
 Con-isLattice = record  { isPartialOrder  = ≤-isPartialOrder {𝑨 = 𝑨} {L}
                         ; supremum        = Con-supremum
                         ; infimum         = ∧-infimum {𝑨 = 𝑨} {L}
                         }
...
The bottom congruence `⊥c` is the *least* congruence, which we obtain for free as the
congruence generated by the empty relation: `⊥c = Cg ∅`.  Its minimality is then
immediate from `Cg-least` (the empty relation is contained in every congruence).  The
top congruence `⊤c` is the all-relation, which is trivially a congruence and clearly
the greatest.
...
```agda
 ...
 ⊤c : Conᴸ
 ⊤c = ∇ᴿ , mkcon (λ _ → lift tt)
                 (record { refl = lift tt ; sym = λ _ → lift tt ; trans = λ _ _ → lift tt })
                 (λ _ _ → lift tt)

 ⊥-minimum : Minimum (_≤_ {𝑨 = 𝑨} {L}) ⊥c
 ⊥-minimum θ = Cg-least {𝑨 = 𝑨} θ (λ { (lift ()) })

 ⊤-maximum : Maximum (_≤_ {𝑨 = 𝑨} {L}) ⊤c
 ⊤-maximum θ _ = lift tt
```

With the bounds the lattice becomes a bounded lattice (`⊤` is the maximum, `⊥` the
minimum).

```agda
 Con-isBoundedLattice : IsBoundedLattice (_≅_ {𝑨 = 𝑨} {L}) _≤_ _∨_ _∧_ ⊤c ⊥c
...
```agda
 ...
 Con-CompleteLattice = record
  ...
  ; _≈ᶜ_             = _≅_ {𝑨 = 𝑨} {L}
  ; _≤ᶜ_             = _≤_ {𝑨 = 𝑨} {L}
  ; isPartialOrderᶜ  = ≤-isPartialOrder {𝑨 = 𝑨} {L}
  ; ⨆                = ⋁
  ; ⨅                = ⋀
...
<span style="float:left;">[← Setoid.Algebras.Congruences.Generation](Setoid.Algebras.Congruences.Generation.html)</span>
