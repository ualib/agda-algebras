...
The bottom congruence `0ᴸ` is the *least* congruence, which we obtain for free as the
congruence generated by the empty relation: `0ᴸ = Cg 0ᴬ`.  Its minimality is then
immediate from `Cg-least` (the empty relation is contained in every congruence).  The
top congruence `1ᴸ` is the all-relation, which is trivially a congruence and clearly
the greatest.
