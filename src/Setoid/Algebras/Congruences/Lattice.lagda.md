...
#### <a id="the-congruence-lattice-of-a-setoid-algebra">The Congruence Lattice of a Setoid Algebra</a>

This is the [Setoid.Algebras.Congruences.Lattice][] module of the [Agda Universal Algebra Library][].

The congruences of an algebra `𝑨`, ordered by containment, form a complete
lattice.  This module begins the formalization of that fact (issue
[#271][]) by promoting `Con 𝑨` to a first-class ordered object: it defines the
containment order `_≤_`, the induced equivalence `_≅_` of mutual containment,
and the **meet** `θ ∧ φ`, which is the relational intersection `θ ∩ φ`.  The
intersection of two congruences is again a congruence (the "tractable half" of
the lattice), and it is the greatest lower bound of the two arguments, so the
order and the meet assemble into stdlib `Poset` and `MeetSemilattice` bundles.

The *join* `θ ∨ φ = Cg(θ ∪ φ)` — the congruence generated by the union — and the
infinitary operations needed for the complete lattice are deferred to the
subsequent steps of #271, since the join requires the congruence-generation
...
#### <a id="the-containment-order">The containment order on congruences</a>

For congruences `θ φ : Con 𝑨` we set `θ ≤ φ` to mean that the underlying relation
of `θ` is contained in that of `φ`.  This is a preorder; it is antisymmetric only
up to the equivalence `_≅_` of *mutual* containment (relation equality is not
propositional), so `_≤_` is a partial order with respect to `_≅_`.
...
#### <a id="meet">Meet: the intersection of two congruences</a>
