...
lattice: it asks how two congruences sit relative to one another under **relation
composition**, an operation that is not part of the lattice structure and does not in
general return a congruence.
...
reverse inclusion is the same statement with `θ` and `φ` swapped, asking that *every*
pair of congruences permute is asking that composition be commutative on `Con 𝑨`.
An algebra (or a variety) with this property is **congruence-permutable**, the
property that the classical *Maltsev condition* characterizes by a single ternary
term ([Setoid.Varieties.Maltsev][], [Setoid.Varieties.MaltsevConditions][]).
...
```agda
  -- θ ⊆ θ ⨾ φ : take the witness y and close on the right by reflexivity of φ.
  ⨾-inˡ : (θ φ : Con 𝑨 ℓ){x y : 𝕌[ 𝑨 ]} → proj₁ θ x y → (θ ⨾ φ) x y
  ⨾-inˡ θ φ {x}{y} xθy = y , xθy , IsEquivalence.refl (is-equivalence (proj₂ φ))

  -- φ ⊆ θ ⨾ φ : take the witness x and close on the left by reflexivity of θ.
...
Two congruences **permute** when `θ ⨾ φ` is contained in `φ ⨾ θ`.  (Here `_⊆_` is the
standard-library relation-inclusion `_⇒_`: `R ⊆ S` means `∀ {x y} → R x y → S x y`.)
...
`Permutes φ θ` we get the two inclusions making `θ ⨾ φ` and `φ ⨾ θ` mutually
contained.  Hence in a congruence-permutable algebra composition is genuinely
commutative on congruences — the conventional statement `θ ⨾ φ = φ ⨾ θ`, read here
as mutual containment, the setoid notion of equal relations.
