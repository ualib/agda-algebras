...
module _ (𝑨 : Algebra α ρ) (ℓ₀ : Level) where
  open Setoid 𝔻[ 𝑨 ] using ( _≈_ ) renaming ( refl to reflA ; reflexive to reflexiveA )
...
congruence generated by the diagonal relation: `0ᴬ = Cg 0A`.  Its minimality is then
...
```agda
 -- The empty and full relations at level L.
  ...
  0ᴬ-minimum θ = Cg-least θ ( (λ x  → reflexive (θ .proj₂) (x .lower)))
