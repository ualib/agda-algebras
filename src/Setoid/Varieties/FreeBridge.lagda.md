...
open import Setoid.Varieties.SoundAndComplete {𝑆 = 𝑆}  using ( Eq ; _≈̇_ ; _⊢_▹_≈_ ; module FreeAlgebra )
...
```agda
  ...
  cg-pair→⊢ : (σ : Sub Y X)(a b : Term X)
    → E ⊢ Y ▹ a [ σ ] ≈ b [ σ ]
    → {s t : Term X} → Gen ❴ a , b ❵ s t → E ⊢ Y ▹ s [ σ ] ≈ t [ σ ]
...
module _
  ...
  recover : {s t : Term X} → Gen ❴ ℊ u , ℊ v ❵ s t → E ⊢ X ▹ s [ σ ] ≈ t [ σ ]
