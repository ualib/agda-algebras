...
The term algebra `𝑻 X`{.AgdaFunction} is free among *all* algebras.  What the HSP
theorem needs is an algebra free among the members of a given class `𝒦`, and that
is what `𝔽[_]`{.AgdaFunction}, the **relatively free algebra**, provides: the
quotient of `𝑻 X` by the equations that hold throughout `𝒦`.  Two terms are
identified in `𝔽[ X ]` exactly when every algebra of `𝒦` satisfies the equation
between them, so `𝔽[ X ]` satisfies precisely the identities of `𝒦` and no more.

This module constructs it, together with the natural epimorphism
`epi𝔽[_]`{.AgdaFunction} from `𝑻 X` onto it and its homomorphism reduct
`hom𝔽[_]`{.AgdaFunction}, and identifies the kernel of that epimorphism with the
theory of the variety generated by `𝒦` (`class-models-kernel`{.AgdaFunction},
`kernel-in-theory`{.AgdaFunction}).  `𝔽-ModTh-epi`{.AgdaFunction} then supplies
the step the theorem turns on: every model `𝑨` of the theory of `V 𝒦` is a
homomorphic image of the relatively free algebra **on its own carrier**,
`𝔽[ ∣A∣ ]`, not of `𝔽[ X ]` for an arbitrary `X`.  The distinction matters: an
arbitrary generator type need not surject onto `𝑨`, whereas taking the generators
to be the elements of `𝑨` makes the epimorphism immediate.  See
[Setoid.Varieties.HSP][] for the argument these pieces are assembled into.
