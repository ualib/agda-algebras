...
a theory interpretation `Th-Day n ≼ ℰ` — states Day's theorem, and proves its
**converse** (hard) direction: a congruence-modular variety has Day terms
...
remains deferred for the substantive structural reason recorded below and in the design
note.
...
module _ {𝑆 : Signature 0ℓ 0ℓ}{X : Type 0ℓ}{Idx : Type ι}
  ...
  -- The converse (hard) half of Day's theorem: a congruence-modular variety
  -- over a finitary signature has a chain of Day terms.
  CM⇒day : Finitary {𝑆 = 𝑆}
    → CongruenceModularVariety {α = lsuc 0ℓ}{ρ = ι ⊔ lsuc 0ℓ}{ℓ = 0ℓ} ℰ
    → Σ[ n ∈ ℕ ] HasDayTerms n (lsuc 0ℓ) (ι ⊔ lsuc 0ℓ) ℰ
  CM⇒day fin cmv = n , I , red
    ...
    xδu : proj₁ (θ ∨ (φ ∧ ψ)) x u
    xδu = proj₂ (𝔽cm θ φ ψ θ⊆ψ) (xθ∨φu , xψu)
    ...
    abstract
      pc : ParityChain 𝔽 (proj₁ (φ ∧ ψ)) (proj₁ θ) x u
      pc = chain→parityᵒ θ (φ ∧ ψ)
             (finitary⇒JoinIsChain {𝑩 = 𝔽} fin θ (φ ∧ ψ) xδu)
    ...
    I : Interpretation (Sig-Day n) 𝑆
    I i = t i

    -- the generators of the Day signature (the source side of I)
    xD yD zD uD : Term {𝑆 = Sig-Day n} (Fin 4)
    ...
    mxyzu mxyyx mxxuu mxyyu : Fin (suc n) → Term {𝑆 = Sig-Day n} (Fin 4)
    ...
    red : (𝑩 : Algebra (lsuc 0ℓ) (ι ⊔ lsuc 0ℓ)) → 𝑩 ⊨ₑ ℰ → reductᴵ 𝑩 I ⊨ₑ Th-Day n
