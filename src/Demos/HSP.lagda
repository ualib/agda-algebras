...
\T{X}~\af{/}~\afld{≈} where \ab x~\afld{≈}~\ab y if and only if the pair (\ab x,~\ab y) belongs to all congruences \ab{θ}~\as{∈}~\ab{Con}~\ab{𝑨} where \ab{𝑨} ranges over homomorphic images of algebras in \af{S} \ab{𝒦}.
...
\AgdaBound{𝒦}\AgdaSymbol{)}, the epimorphism \af{T}~\ab{A} \as{→} \ab{A}
factors through \T{X}~\as{→}~\Free{X}.
%(Here, \ab{A} denotes the carrier \aof{𝕌[}~\ab{𝑨}~\aof{]} of the algebra \ab{𝑨}.)  
Therefore, \Free{X} is a subdirect product of the algebras in \{\T{X}~\af{/}~\afld{≈}\}.
Therefore, \ab x~\afld{≈}~\ab y if and only if
(\ab x, \ab y) belongs to the kernel of every homomorphism into a member of \af{S} \ab{𝒦}.
%In other terms, \ab x \afld{≈} \ab y iff (\ab x, \ab y) ∈ \af{Th} \ab{𝒦}. 
\begin{comment}
let \ab{ℰ} = \af{Th} \ab{𝒦} (the set of equations modeled by \ab{𝒦}) and let \afld{≈} be the equivalence relation on \Term{X} generated by \ab{ℰ}. Define the relatively free algebra over \ab{X}
%(relative to \ab{𝒦})
...
%%% Below we will see how to construct the equivalence \afld{≈} and the quotient
%%% \Term{X}~\af{/}~\afld{≈} in \agda. as the setoid with carrier \Term{X} and equivalence relation~\afld{≈}.
...
\Free{X} \af{∈} \af{S}(\af{P}(\af{S}~\ab{𝒦})) ⊆ \af{S}(\af{P}~\ab{𝒦}).
\end{comment}
...
\ab q.   More precisely, the equivalence relation generated by \ab{ℰ} includes all pairs (\ab p , \ab q) of \ab{𝑆}-terms such that \ab p \aod{≈} \ab q is derivable from \ab{ℰ}.
%; that is, \ab{ℰ} \aod{⊢} \ab X \aod{▹} \ab p \aod{≈} \ab q.
