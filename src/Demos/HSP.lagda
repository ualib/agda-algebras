...
library,~\cite{DeMeo_agda-algebras:2021}---is fully constructive and correct.  To the
best of our knowledge, ours is the first formulation of the HSP theorem in MLTT, and the
...
theorems required postulating function extensionality, an axiom that is neither provable
nor disprovable in pure Martin-Löf type theory~(\cite{MHE}).
...
assumptions or inconsistencies that could be used to fool a type-checker.\footnote{As of 26 Nov 2021, the latest version of \agdaalgebras is 2.0.0; see~\cite{DeMeo_agda-algebras:2021}.}
...
It is easy to prove that \ar{_≅_} is an equivalence relation, as follows.
