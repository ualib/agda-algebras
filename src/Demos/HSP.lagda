...
underlying domain setoid. Concretely, an algebra of type \ar{Algebra} \ab{α} \ab{ρᵃ} has a
...
\af{Rel} \afld{Carrier} \ab{ρᵃ}. Now, examining the \af{Lift-Alg} function, we see that it
takes an algebra---one whose carrier inhabits \ap{Type \ab{α}} with equality
of type \af{Rel} \afld{Carrier} \ab{ρᵃ}---and constructs a new algebra whose carrier
inhabits \ap{Type} (\ab{α} \ap{⊔} \ab{ℓ₀}) with equality of type \af{Rel} \afld{Carrier}
(\ab{ρᵃ} \ap{⊔} \ab{ℓ₁}). This lifting operation would be worthless without a useful
...
a fact we will codify later in a type called \af{Lift-≅}.

\paragraph*{Product Algebras}
We give an informal description of the \defn{product} of a family of \ab{𝑆}-algebras and
...
image.\footnote{Here and in \agdaalgebras we use the shorthand \af{ov}~\ab{α} := \ab{𝒪}
...
\ab{𝑇ₙ}. (Recall, \aof{∥~\ab{𝑆}~∥} \ab f is the arity of the operation symbol \ab
f.) An \ab{𝑆}-\defn{term} is a term in the language of \ab{𝑆} and the collection of all
...
(\aic{ℊ} for ``generator'' and \aic{node} for ``node''). We enrich the \ad{Term} type with
...
of an algebra---the (absolutely free) \emph{term algebra} in the signature \ab{𝑆}.
...
which denotes a substitution \ab{σ} applied to a term \ab t.
...
The \defn{substitution lemma} says that applying a substitution \ab{σ} to a term \ab{t}
has the same effect on the meaning of \ab{t} as changing the environment so that each
subterm \ab{x} has the value \aof{⟦~\ab{σ}~\ab{x}~⟧}~\aofld{⟨\$⟩} \ab{ρ}. (See, e.g.,~\cite[Lemma 3.3.11]{Mitchell:1996}.)
