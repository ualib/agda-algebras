...
IsSubuniverse : (A : algebra S) (B : Pred ⟦ A ⟧ᵤ ℓ) -> Set ℓ
-- data _SUB_ : {A : algebra S} (X : Pred ⟦ A ⟧ᵤ ℓ) -> Set ℓ where
...
open _≃_
record _sg_ {A : algebra S} (X : ⟦ A ⟧ᵤ -> Prp) (Y : ⟦ A ⟧ᵤ -> Prp) : Set _ where
  field
    ss : X ⊆ Y
    su 
  gen : ∀ {x : X}
