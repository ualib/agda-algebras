...
This is the M4-5d spike (#342): the *free expansion* along the symbol-adjoining
signature inclusion `Sig-Magma ↪ Sig-Monoid`, in its classical concrete instance —
freely adjoining a unit to a semigroup, i.e. the free monoid on a semigroup — proved
to be left adjoint to the reduct-derived forgetful functor, with unit, counit, both
naturality squares, and the two triangle identities all type-checked, plus the
explicit universal property (existence and uniqueness of the extension).
...
unit laws hold by computation and associativity reduces to the semigroup's.  This is
the sharp contrast with M3-6's `expand-ε` ([`Classical.Structures.Monoid`][]), which
interprets `ε-Op` as a *chosen element of the existing carrier* — a section of the
reduct (`opsToBareMonoid-section`), not its left adjoint.  See
`docs/notes/m4-5d-free-expansion.md` for the full comparison and for why the spike
lives at the theory-satisfying (bundle) level: it is precisely the monoid *equations*
that collapse the free expansion to first-order normal forms (`Maybe`), with no
setoid quotient; the equation-free expansion on raw algebra categories needs the
term-algebra quotient and is deferred.

The categorical scaffolding is the minimal self-contained vocabulary of ADR-006:
...
theory-satisfying algebras is just a homomorphism of the underlying algebras — and
the forgetful functor is [`reductF`][Classical.Categories.Reduct] restricted along
`monoid→semigroup`'s theory transfer.  The module lives in `Classical.Categories`
because its objects are the `Classical` bundles and its right adjoint's object map
is `reduct` (the placement rule of ADR-006).
...
open import Setoid.Categories.FullSubcategory  using ( FullSub ; FullSubF )
...
Semigroups α ρ = FullSub (AlgCat.Alg {𝑆 = Sig-Magma} α ρ) (_⊨ˢᵍ Th-Semigroup)

Monoids : (α ρ : Level) → Category (suc (α ⊔ ρ)) (α ⊔ ρ) (α ⊔ ρ)
Monoids α ρ = FullSub (AlgCat.Alg {𝑆 = Sig-Monoid} α ρ) (_⊨ᵐᵒ Th-Monoid)
...
forgetUnitF = FullSubF (reductF magma↪monoid) (λ {𝑨} thm → proj₂ (monoid→semigroup (𝑨 , thm)))

forgetUnitF-F₀ : (ℳ : Monoid α ρ) → Functor.F₀ (forgetUnitF {α} {ρ}) ℳ ≡ monoid→semigroup ℳ
forgetUnitF-F₀ _ = refl
```
...
module _ (𝑺 𝑻 : Semigroup α ρ) where
  private
    module S = AdjoinUnit 𝑺
    module T = AdjoinUnit 𝑻
    𝑨 = proj₁ 𝑺
    𝑩 = proj₁ 𝑻
  open Setoid 𝔻[ 𝑩 ] using () renaming ( _≈_ to _≈ᵇ_ ; refl to ≈ᵇ-refl )

  map¹ : Func 𝔻[ 𝑨 ] 𝔻[ 𝑩 ] → Func S.𝔻¹ T.𝔻¹
  ...
  map¹-∙ : (f : hom 𝑨 𝑩) (s t : S.A¹)
    → map¹ (proj₁ f) ⟨$⟩ (s S.∙¹ t) T.≈¹ (map¹ (proj₁ f) ⟨$⟩ s) T.∙¹ (map¹ (proj₁ f) ⟨$⟩ t)
  map¹-∙ f (just x) (just y) = hom-preserves-∙ f x y
  map¹-∙ f (just _) nothing  = ≈ᵇ-refl
  map¹-∙ f nothing  t        = T.≈¹-refl (map¹ (proj₁ f) ⟨$⟩ t)

  map¹-compatible : (f : hom 𝑨 𝑩) (o : Op-Monoid) (args : ArityOf Sig-Monoid o → S.A¹)
    → map¹ (proj₁ f) ⟨$⟩ ((o ^ S.𝑨¹) args) T.≈¹ (o ^ T.𝑨¹) (λ i → map¹ (proj₁ f) ⟨$⟩ args i)
  map¹-compatible f ∙-Op args = map¹-∙ f (args 0F) (args 1F)
  map¹-compatible f ε-Op args = tt

  adjoinUnit-hom : hom 𝑨 𝑩 → hom S.𝑨¹ T.𝑨¹
  adjoinUnit-hom f = map¹ (proj₁ f) , record { compatible = λ {o} {args} → map¹-compatible f o args }

  -- map¹ respects the pointwise hom-equality.
  adjoinUnit-resp : {f g : hom 𝑨 𝑩} → ((x : 𝕌[ 𝑨 ]) → proj₁ f ⟨$⟩ x ≈ᵇ proj₁ g ⟨$⟩ x)
    → (t : S.A¹) → map¹ (proj₁ f) ⟨$⟩ t T.≈¹ map¹ (proj₁ g) ⟨$⟩ t
...
module _ (𝑺 : Semigroup α ρ) where
  private module S = AdjoinUnit 𝑺
  open Setoid 𝔻[ proj₁ 𝑺 ] using () renaming ( refl to ≈ˢ-refl )

  adjoinUnit-id : (t : S.A¹) → map¹ 𝑺 𝑺 (proj₁ (𝒾𝒹 {𝑨 = proj₁ 𝑺})) ⟨$⟩ t S.≈¹ t
...
module _ (𝑺 𝑻 𝑼 : Semigroup α ρ) (f : hom (proj₁ 𝑺) (proj₁ 𝑻)) (g : hom (proj₁ 𝑻) (proj₁ 𝑼)) where
  private
    module S = AdjoinUnit 𝑺
    module U = AdjoinUnit 𝑼
  open Setoid 𝔻[ proj₁ 𝑼 ] using () renaming ( refl to ≈ᵘ-refl )

  adjoinUnit-∘ : (t : S.A¹)
    → map¹ 𝑺 𝑼 (proj₁ (⊙-hom f g)) ⟨$⟩ t U.≈¹ map¹ 𝑻 𝑼 (proj₁ g) ⟨$⟩ (map¹ 𝑺 𝑻 (proj₁ f) ⟨$⟩ t)
...
module _ (𝑺 : Semigroup α ρ) (𝑴 : Monoid α ρ) (f : hom (proj₁ 𝑺) (proj₁ (monoid→semigroup 𝑴))) where
  ...
  -- Formal products in the expansion become real products in 𝑴; the unit laws of
  -- 𝑴 absorb the cases involving the fresh unit.
  extend-∙ : (s t : S.A¹)
    → extend-map ⟨$⟩ (s S.∙¹ t) ≈ᵇ (extend-map ⟨$⟩ s) ∙ᵐ (extend-map ⟨$⟩ t)
  ...
  extend-compatible : (o : Op-Monoid) (args : ArityOf Sig-Monoid o → S.A¹)
    → extend-map ⟨$⟩ ((o ^ S.𝑨¹) args) ≈ᵇ (o ^ 𝑩) (λ i → extend-map ⟨$⟩ args i)
  ...
  extend-unique : (g : hom S.𝑨¹ 𝑩) → ((x : 𝕌[ 𝑨 ]) → proj₁ g ⟨$⟩ just x ≈ᵇ proj₁ f ⟨$⟩ x)
    → (t : S.A¹) → proj₁ g ⟨$⟩ t ≈ᵇ extend-map ⟨$⟩ t
...
module _ (𝑺 : Semigroup α ρ) where
  ...
  zig¹ : (t : S.A¹)
    → proj₁ (counit-hom F𝑺) ⟨$⟩ (map¹ 𝑺 (monoid→semigroup F𝑺) (unit-map 𝑺) ⟨$⟩ t) S.≈¹ t
  zig¹ (just _) = ≈ˢ-refl
  zig¹ nothing  = tt
```
