...
### <a id="classical-structures-commutativemonoid">Commutative monoids</a>
...
open import Classical.Structures.Monoid            using ( Monoid ; module Monoid-Op ; fromMonoidOps )
...
#### <a id="op">The `CommutativeMonoid-Op` module</a>
...
```agda
fromCommMonoidEqs : (A : Type α) (_·_ : A → A → A) (e : A)
  → (·-assoc : ∀ a b c → (a · b) · c ≡ a · (b · c))
  → (·-idˡ : ∀ a → e · a ≡ a) (·-idʳ : ∀ a → a · e ≡ a)
  → (·-comm : ∀ a b → a · b ≡ b · a)
  → CommutativeMonoid α α
fromCommMonoidEqs A _·_ e ·-assoc ·-idˡ ·-idʳ ·-comm = fromMonoidOps A _·_ e , proof
  where
  proof : (fromMonoidOps A _·_ e) ⊨ᶜᵐᵒ Th-CommutativeMonoid
