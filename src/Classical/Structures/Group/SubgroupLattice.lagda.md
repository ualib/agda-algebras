...
```agda
module GroupSublattice {α ρ : Level} (𝑮 : Group α ρ) (ℓ₀ : Level) where
  private
    𝑨 = proj₁ 𝑮
    A = 𝕌[ 𝑨 ]

  open Group-Op 𝑮 using ( ε )

  open Sublattice 𝑨 ℓ₀ public
```
...
```agda
  ...
  ε-mem B = sub-ε-closed 𝑮 {B = proj₁ B} (proj₂ B)
```
...
```agda
  ...
  subgroup→Subᴸ : (B : Pred A L) → IsSubgroup 𝑮 B → Subᴸ
...
```agda
  ...
  ∧-isSubgroup : (B C : Subᴸ)
    →  IsSubgroup 𝑮 (proj₁ B) → IsSubgroup 𝑮 (proj₁ C)
    →  IsSubgroup 𝑮 (proj₁ (B ∧ C))
