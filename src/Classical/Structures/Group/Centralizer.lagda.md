...
antitone in `N`, and it is *normal* whenever `N` is — the three facts the structure
theory of parachute representations needs.[^1]

The module also proves the small commutator fact that drives subdirect
...
module Centralizer {α ρ : Level} (𝒢 : Group α ρ) where
  ...
  C-antitone : {M : Pred G ℓ} {N : Pred G ℓ'} → M ⊆ N → C[ N ] ⊆ C[ M ]
  C-antitone M⊆N g∈C x x∈M = g∈C x (M⊆N x∈M)
```
...
```agda
  ...
  C-normal {N = N} N-normal k {g} g∈C x x∈N = begin
