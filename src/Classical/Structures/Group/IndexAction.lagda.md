...
This is the gadget Kurzweil's wreath construction consumes: the multiplication
of `S ≀ G` twists the base tuple of the right factor by the action of the left
factor's top component — the display `(s , x) (t , y) = (s₁ t_{x(1)}, …, x y)`
of the note[^1] reads coordinate `i` of the product as `s i ∙ t (x(i))`, and
associativity of the wreath needs exactly the contravariant law
`(x ∙ y)(i) = y (x (i))`.

Three design points.

+  **The index set is a bare type**, acted on up to propositional equality `≡` —
   not a setoid.  The intended instances are finite index sets `Fin n` (the
   cosets of a finite-index subgroup, enumerated), where `≡` is the right
   equality; keeping the index side propositional lets tuples `I → S` be
   permuted by plain precomposition, with no `Func` bookkeeping.  The group
   side *is* a setoid, so the action carries a congruence field.
+  **No bijectivity field**.  Invertibility of each `act x` is a consequence of
   ...
   bijections.
+  **Relation to [Classical.Structures.Group.GSet][]**.  The library's G-sets
   present the coset action as a unary algebra on the coset *setoid* (one
   operation per group element); that form feeds the congruence bridge of
   [FLRP.Bridge][].  The present module is the *enumerated* counterpart: an
   action on a bare index set, which is what powers `Sᴵ` and wreath products
   index over.  The two meet in the coset-action specification below.

The module closes with the **kernel–core correspondence**: for an action
satisfying the (pointed) coset-action specification for a subgroup `H`, the
action is faithful precisely when `H` is core-free — the fact `ker φ = 1 ⟺
Core_G(H) = 1` that the wreath no-go argument of RP-4 (issue #461) turns on.
Both directions are proved outright; nothing here is assumed.
...
`RightAction`{.AgdaRecord} `I` `𝒢` packages an action of `𝒢`{.AgdaBound} on the
index type `I`{.AgdaBound}: the map `act`{.AgdaField}, its congruence in the
group argument (the group carrier is a setoid, and `≈`-equal elements must act
identically), the identity law, and the contravariant compatibility law.
...
cosets of a subgroup `H`, enumerated by an index set.  We specify it *pointedly*:
some index plays the role of the coset `H` itself — its stabilizer is exactly
`H`, in both directions — and every index is reachable from it (transitivity).
This ties the abstract action to the pair `(G , H)` up to isomorphism of
G-sets, which is all any consumer needs, while staying agnostic about how the
cosets were enumerated.

```agda
record IsCosetAction {I : Type ι} {𝒢 : Group β σ}
  (A : RightAction I 𝒢) (H : Pred 𝕌[ proj₁ 𝒢 ] ℓ) : Type (ι ⊔ β ⊔ σ ⊔ ℓ)
  where
  open RightAction A
  ...
  field
    ...
    reach      : ∀ i → Σ[ g ∈ 𝕌[ proj₁ 𝒢 ] ] (act g basepoint ≡ i)
```
...
normal core `Core_G(H)`.  Both directions are proved outright below, against
the library's constructive core (the meet of all conjugates,
[Classical.Structures.Group.NormalCore][]); "core-free" is the containment of
the core in the ≈-class of the identity, exactly the form
`CoreFree`{.AgdaFunction} of [FLRP.Enforceable][] unfolds to.

`ActionKernel`{.AgdaModule} fixes the data once for both directions.

```agda
module ActionKernel {I : Type ι} (𝒢 : Group β σ)
  (H : Pred 𝕌[ proj₁ 𝒢 ] ℓ) (H-sg : IsSubgroup 𝒢 H)
  ...
  open Group-Op 𝒢      using ( _∙_ ; ε ; _⁻¹ ; assoc-law ; idʳ-law ; invˡ-law
                             ; ∙-cong )
...
One group-algebra step is shared by the second direction: multiplying a
conjugate `g ∙ x ∙ g ⁻¹` back by `g` on the right recovers `g ∙ x`.

```agda
  private
    -- (g ∙ x ∙ g ⁻¹) ∙ g ≈ g ∙ x, by reassociating and cancelling the inverse.
    conj-slide : ∀ g x → (conj g x) ∙ g ≈ g ∙ x
...
```agda
  coreFree→faithful : proj₁ core ⊆ proj₁ (trivialSubgroup 𝒢) → Faithful
  coreFree→faithful cf {x} fix = cf (conj-mem-core conj∈H)
    ...
    conj∈H g = stab-out (trans (act-∙ (g ∙ x) (g ⁻¹) basepoint)
      (trans  (cong (act (g ⁻¹))
                    (trans (act-∙ g x basepoint) (fix (act g basepoint))))
...
```agda
  faithful→coreFree : Faithful → proj₁ core ⊆ proj₁ (trivialSubgroup 𝒢)
    ...
    fix i = trans (cong (act x) (sym gb≡i))
      (trans  (sym (act-∙ g x basepoint))
        (trans  (act-cong (≈sym (conj-slide g x)) basepoint)
          (trans  (act-∙ (conj g x) g basepoint)
