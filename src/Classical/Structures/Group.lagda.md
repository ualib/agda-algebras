...
[`Classical.Structures.Monoid`][]'s `monoid→semigroup` (here extended from one law to
three, the two identity laws additionally bridging the nullary `ε-Op` node).
...
```agda
infix 4 _⊨ᵍʳ_
_⊨ᵍʳ_ : (𝑨 : Algebra α ρ) (ℰ : Eq-Group → Term (Fin 3) × Term (Fin 3)) → Type (α ⊔ ρ)
𝑨 ⊨ᵍʳ ℰ = ∀ i → 𝑨 ⊧ proj₁ (ℰ i) ≈ proj₂ (ℰ i)
```
...
Group α ρ = Σ[ 𝑨 ∈ Algebra α ρ ] 𝑨 ⊨ᵍʳ Th-Group
```
...
module Group-Op {α ρ : Level} (𝑮 : Group α ρ) where
  ...
  invˡ-law : ∀ x → (x ⁻¹) ∙ x ≈ ε
  invˡ-law x = trans (∙-cong (sym (interp-node-⁻¹ (ℊ 0F) {λ _ → x})) refl)
                     (trans (sym (interp-node-∙ (node ⁻¹-Op (λ _ → ℊ 0F)) (ℊ 0F) {λ _ → x}))
                            (trans (equations invˡ (λ _ → x)) (interp-node-ε {λ _ → x})))
...
group→monoid 𝒢@(𝑮 , _) = 𝑹 , thm
  ...
  thm : 𝑹 ⊨ᵐᵒ Th-Monoid
  thm assocᵐ η = let x = η 0F ; y = η 1F ; z = η 2F in
    begin
...
opsToBareGroup : (A : Type α) (_·_ : A → A → A) (e : A) (i : A → A)
  → Algebra {𝑆 = Sig-Group} α α
opsToBareGroup A _·_ e i .Domain = ≡.setoid A
opsToBareGroup A _·_ e i .Interp ⟨$⟩ (∙-Op  , args) = args 0F · args 1F
opsToBareGroup A _·_ e i .Interp ⟨$⟩ (ε-Op  , _)    = e
opsToBareGroup A _·_ e i .Interp ⟨$⟩ (⁻¹-Op , args) = i (args 0F)
opsToBareGroup A _·_ e i .Interp .cong {∙-Op  , _} {.∙-Op  , _} (≡.refl , u≈v) = ≡.cong₂ _·_ (u≈v 0F) (u≈v 1F)
opsToBareGroup A _·_ e i .Interp .cong {ε-Op  , _} {.ε-Op  , _} (≡.refl , _)   = ≡.refl
opsToBareGroup A _·_ e i .Interp .cong {⁻¹-Op , _} {.⁻¹-Op , _} (≡.refl , u≈v) = ≡.cong i (u≈v 0F)
```
...
eqsToGroup : (A : Type α) (_·_ : A → A → A) (e : A) (i : A → A)
  ...
  → Group α α
eqsToGroup A _·_ e i ·-assoc ·-idˡ ·-idʳ ·-invˡ ·-invʳ = opsToBareGroup A _·_ e i , proof
  where
  proof : opsToBareGroup A _·_ e i ⊨ᵍʳ Th-Group
