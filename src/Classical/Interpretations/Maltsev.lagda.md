...
The Maltsev condition *as universal algebra* — the abstract one-ternary-symbol theory
`Th-Maltsev`, and the predicate `HasMaltsevTerm ℰ = Th-Maltsev ≼ ℰ` framing it as a theory
interpretation — lives in [Setoid.Varieties.Maltsev][].  This module supplies a *worked
witness* for one concrete variety: the variety of **groups**.  Because the proof consumes
the group operations and laws (`Classical.Structures.Group`), it is structure-specific and
so belongs in `Classical/`, one layer above the general theory — the same split as the
general [Setoid.Varieties.Invariance][] versus the group-specific
[Classical.Categories.Forgetful][].

In any group, `m(x, y, z) = x ∙ (y ⁻¹ ∙ z)` is a Maltsev term, since `x ∙ (x ⁻¹ ∙ z) = z`
and `x ∙ (y ⁻¹ ∙ y) = x`.  The interpretation `I-grp` sends `m-Op` to that derived term,
and the milestone's satisfaction condition (`⊧-interp`) reduces the obligation
...
open import Setoid.Varieties.Interpretation
  using ( reductᴵ ; ⊧-interp ; graft-eval ; _⊨ₑ_ )
open import Setoid.Varieties.Maltsev
  using ( Sig-Maltsev ; m-Op ; tri ; mlt ; malˡ ; malʳ ; Th-Maltsev ; HasMaltsevTerm )
...
```agda
maltsev-≼-group : ∀ {α ρ} → HasMaltsevTerm {α = α} {ρ = ρ} Th-Group
