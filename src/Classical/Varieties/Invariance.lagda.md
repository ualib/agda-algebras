...
This module proves the *reduct-invariance of satisfaction*, the payoff theorem of the
M4-5 chain: for a signature morphism `φ : 𝑆₁ → 𝑆₂`, an `𝑆₂`-algebra `𝑨`, and
`𝑆₁`-terms `s , t`,
...
against `𝑨` itself.  Model theorists know this as (the equational case of) the
**satisfaction condition** of institutions — Goguen and Burstall's slogan is "truth
is invariant under change of notation" — and universal algebraists use it tacitly
every time they say "a monoid satisfies the semigroup laws."
...
needed).  Evaluation of `𝑆₁`-terms in the reduct, and of `𝑆₂`-terms in `𝑨`, fit
around the term translation:
...
import Setoid.Terms.Basic                  as TermsBasic
...
module _ {𝑆₁ 𝑆₂ : Signature 𝓞 𝓥} (φ : SigMorphism 𝑆₁ 𝑆₂) (𝑨 : Algebra {𝑆 = 𝑆₂} α ρ) where
  private
    module B₁ = TermsBasic {𝑆 = 𝑆₁}
    module B₂ = TermsBasic {𝑆 = 𝑆₂}
    module E₁ = B₁.Environment (reduct φ 𝑨)
    module E₂ = B₂.Environment 𝑨
...
```agda
  reduct-interp : (t : Term {𝑆 = 𝑆₁} X) (η : X → 𝕌[ 𝑨 ])
    → E₁.⟦ t ⟧ ⟨$⟩ η ≈ E₂.⟦ φ ✶ t ⟧ ⟨$⟩ η
...
```agda
  ⊧-reduct : {s t : Term {𝑆 = 𝑆₁} X}
    → 𝑨 ⊧₂ (φ ✶ s) ≈ (φ ✶ t) → reduct φ 𝑨 ⊧₁ s ≈ t
  ⊧-reduct {s = s} {t} A⊧ η =
    trans (reduct-interp s η) (trans (A⊧ η) (sym (reduct-interp t η)))

  ⊧-expand : {s t : Term {𝑆 = 𝑆₁} X}
    → reduct φ 𝑨 ⊧₁ s ≈ t → 𝑨 ⊧₂ (φ ✶ s) ≈ (φ ✶ t)
  ⊧-expand {s = s} {t} R⊧ η =
