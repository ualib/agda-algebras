---
layout: default
file: "src/FLRP/Certificates/SmallLatticeReps/SLR06.lagda.md"
title: "FLRP.Certificates.SmallLatticeReps.SLR06 module (The Agda Universal Algebra Library)"
date: "2026-07-22"
author: "the agda-algebras development team (emitted by scripts/python/flrp)"
---

### A machine-checked representation: Con(B6) ≅ manuscript L6 (the hexagon)

This is the [FLRP.Certificates.SmallLatticeReps.SLR06][] module of the [Agda Universal Algebra Library][].

**This module was emitted by `scripts/python/flrp/emit_agda.py` from
`scripts/python/flrp/inputs/slr/slr06.json`.  Do not edit it by hand; rerun the emitter instead.**

`B6`/`L6` in the numbering of the DeMeo–Freese–Jipsen manuscript *Representing Finite Lattices as Congruence Lattices of Finite Algebras* (§ 6 of the 2016-06-10 draft, `docs/papers/fin-lat-rep/SmallLatticeReps.tex`).  The dictionary between the manuscript's lattice numbering and this library's names is `docs/notes/flrp-slr-naming.md`.

It re-verifies, end-to-end through the WP-6 certificate pipeline (#457), the
claim that the congruence lattice of the algebra "B6" — carrier size
6, unary operations `f`, `g`, `h` — is isomorphic to the lattice
"manuscript L6 (the hexagon)" (6 elements).  The engine's output below (normal-form
...
module FLRP.Certificates.SmallLatticeReps.SLR06 where
...
```agda
opTables : Vec (Vec (Fin 6) 6) 3
opTables = (2F ∷ 2F ∷ 1F ∷ 5F ∷ 5F ∷ 4F ∷ [])
         ∷ (3F ∷ 4F ∷ 4F ∷ 0F ∷ 1F ∷ 1F ∷ [])
         ∷ (4F ∷ 5F ∷ 3F ∷ 4F ∷ 5F ∷ 3F ∷ [])
         ∷ []

𝑨 : Algebra {𝑆 = Sig-Unary (Fin 3)} 0ℓ 0ℓ
𝑨 = tablesToUnaryAlgebra 6 3 opTables

𝑭 : FiniteAlgebra 𝑨
𝑭 = tablesToUnaryAlgebra-FiniteAlgebra 6 3 opTables
...
The claimed congruence lattice "manuscript L6 (the hexagon)", presented exactly as the
...
cert = mkLatticeCert partsᵛ 0F prinᵛ prinTrᵛ ∧-table ∨-table joinTrᵛ
  ...
  prinTrᵛ : Vec (Vec (Trace 6 3 arOf) 6) 6
  prinTrᵛ = ( []
          ...
          ∷ ( ( mkMerge 1F 0F (seed 0)
            ...
            ∷ ( mkMerge 1F 2F (seed 0)
              ∷ mkMerge 5F 3F (translate 2F 0F (0F ∷ []) 0)
          ...
          ∷ ( ( mkMerge 2F 0F (seed 0)
            ...
            ∷ ( mkMerge 2F 1F (seed 0)
              ∷ mkMerge 3F 5F (translate 2F 0F (0F ∷ []) 0)
  ...
  joinTrᵛ : Vec (Vec (Trace 6 3 arOf) 6) 6
  joinTrᵛ = ( []
            ∷ ( mkMerge 1F 0F (seed 0)
...
```agda
SLR06-Representableᵈ : Representableᵈ (toLattice 𝑳)
SLR06-Representableᵈ = certRepresentableᵈ 𝑭 𝑺 𝑳 cert certOK certMeet

SLR06-FiniteCongruencesᵈ : FiniteCongruencesᵈ 𝑨
SLR06-FiniteCongruencesᵈ = certFiniteCongruencesᵈ cert certOK
```
