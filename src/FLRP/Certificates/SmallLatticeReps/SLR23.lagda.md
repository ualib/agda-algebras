---
layout: default
file: "src/FLRP/Certificates/SmallLatticeReps/SLR23.lagda.md"
title: "FLRP.Certificates.SmallLatticeReps.SLR23 module (The Agda Universal Algebra Library)"
date: "2026-07-22"
author: "the agda-algebras development team (emitted by scripts/python/flrp)"
---

### A machine-checked representation: Con(B23) ≅ manuscript L23

This is the [FLRP.Certificates.SmallLatticeReps.SLR23][] module of the [Agda Universal Algebra Library][].

**This module was emitted by `scripts/python/flrp/emit_agda.py` from
`scripts/python/flrp/inputs/slr/slr23.json`.  Do not edit it by hand; rerun the emitter instead.**

`B23`/`L23` in the numbering of the DeMeo–Freese–Jipsen manuscript *Representing Finite Lattices as Congruence Lattices of Finite Algebras* (§ 6 of the 2016-06-10 draft, `docs/papers/fin-lat-rep/SmallLatticeReps.tex`).  The dictionary between the manuscript's lattice numbering and this library's names is `docs/notes/flrp-slr-naming.md`.

It re-verifies, end-to-end through the WP-6 certificate pipeline (#457), the
claim that the congruence lattice of the algebra "B23" — carrier size
6, unary operations `f`, `g`, `h`, `k` — is isomorphic to the lattice
"manuscript L23" (7 elements).  The engine's output below (normal-form
...
module FLRP.Certificates.SmallLatticeReps.SLR23 where
...
The claimed congruence lattice "manuscript L23", presented exactly as the
...
cert = mkLatticeCert partsᵛ 0F prinᵛ prinTrᵛ ∧-table ∨-table joinTrᵛ
  ...
  prinᵛ : Vec (Vec (Fin 7) 6) 6
  prinᵛ = (0F ∷ 3F ∷ 5F ∷ 6F ∷ 3F ∷ 6F ∷ [])
        ∷ (3F ∷ 0F ∷ 6F ∷ 1F ∷ 3F ∷ 6F ∷ [])
        ∷ (5F ∷ 6F ∷ 0F ∷ 3F ∷ 6F ∷ 3F ∷ [])
        ∷ (6F ∷ 1F ∷ 3F ∷ 0F ∷ 6F ∷ 3F ∷ [])
        ∷ (3F ∷ 3F ∷ 6F ∷ 6F ∷ 0F ∷ 2F ∷ [])
  ...
  prinTrᵛ = ( []
            ∷ ( mkMerge 0F 1F (seed 0)
              ...
              ∷ mkMerge 3F 5F (translate 3F 0F (0F ∷ []) 1)
              ∷ [] )
            ∷ ( mkMerge 0F 2F (seed 0)
              ∷ mkMerge 1F 3F (translate 1F 0F (0F ∷ []) 0)
              ∷ mkMerge 4F 5F (translate 3F 0F (0F ∷ []) 1)
              ∷ [] )
            ∷ ( mkMerge 0F 3F (seed 0)
              ∷ mkMerge 0F 1F (translate 0F 0F (0F ∷ []) 0)
              ∷ mkMerge 3F 2F (translate 2F 0F (0F ∷ []) 1)
              ∷ mkMerge 4F 3F (translate 3F 0F (0F ∷ []) 2)
              ∷ mkMerge 3F 5F (translate 3F 0F (0F ∷ []) 1)
              ∷ [] )
            ∷ ( mkMerge 0F 4F (seed 0)
              ∷ mkMerge 1F 4F (translate 1F 0F (0F ∷ []) 0)
              ∷ mkMerge 3F 5F (translate 2F 0F (0F ∷ []) 1)
          ...
          ∷ ( ( mkMerge 2F 0F (seed 0)
              ...
              ∷ mkMerge 5F 4F (translate 3F 0F (0F ∷ []) 1)
              ∷ [] )
            ∷ ( mkMerge 2F 1F (seed 0)
              ∷ mkMerge 0F 1F (translate 0F 0F (0F ∷ []) 0)
              ∷ mkMerge 3F 1F (translate 1F 0F (0F ∷ []) 1)
              ∷ mkMerge 5F 1F (translate 3F 0F (0F ∷ []) 2)
          ...
          ∷ ( ( mkMerge 3F 0F (seed 0)
              ∷ mkMerge 1F 0F (translate 0F 0F (0F ∷ []) 0)
              ∷ mkMerge 2F 3F (translate 2F 0F (0F ∷ []) 1)
          ...
          ∷ ( ( mkMerge 4F 0F (seed 0)
              ∷ mkMerge 4F 1F (translate 1F 0F (0F ∷ []) 0)
              ∷ mkMerge 5F 3F (translate 2F 0F (0F ∷ []) 1)
  ...
  joinTrᵛ : Vec (Vec (Trace 6 4 arOf) 7) 7
  joinTrᵛ = ( []
            ∷ (mkMerge 3F 1F (seed 0) ∷ [])
            ∷ (mkMerge 5F 4F (seed 0) ∷ [])
            ∷ ( mkMerge 1F 0F (seed 0)
          ...
          ∷ ( (mkMerge 3F 1F (seed 0) ∷ [])
            ...
            ∷ ( mkMerge 3F 1F (seed 0)
              ...
              ∷ mkMerge 3F 2F (seed 2)
              ∷ mkMerge 4F 0F (seed 3)
              ∷ mkMerge 5F 2F (seed 4)
              ∷ [] )
            ∷ ( mkMerge 3F 1F (seed 0)
              ∷ mkMerge 5F 4F (seed 2)
              ∷ [] )
            ∷ ( mkMerge 3F 1F (seed 0)
              ∷ mkMerge 2F 0F (seed 1)
              ∷ mkMerge 5F 4F (seed 3)
              ∷ [] )
            ∷ ( mkMerge 3F 1F (seed 0)
              ∷ mkMerge 1F 0F (seed 1)
              ∷ mkMerge 2F 0F (seed 2)
              ∷ mkMerge 4F 0F (seed 4)
              ∷ mkMerge 5F 0F (seed 5)
              ∷ [] )
            ∷ [] )
          ∷ ( (mkMerge 5F 4F (seed 0) ∷ [])
            ∷ ( mkMerge 5F 4F (seed 0)
          ...
          ∷ ( ( mkMerge 1F 0F (seed 0)
            ...
            ∷ ( mkMerge 1F 0F (seed 0)
              ...
              ∷ mkMerge 4F 0F (seed 2)
              ∷ mkMerge 5F 2F (seed 3)
              ∷ mkMerge 3F 1F (seed 4)
              ∷ [] )
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
              ∷ mkMerge 4F 0F (seed 2)
              ∷ mkMerge 5F 2F (seed 3)
              ∷ mkMerge 5F 4F (seed 4)
              ∷ [] )
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
              ∷ mkMerge 4F 0F (seed 2)
...
```agda
SLR23-Representableᵈ : Representableᵈ (toLattice 𝑳)
SLR23-Representableᵈ = certRepresentableᵈ 𝑭 𝑺 𝑳 cert certOK certMeet

SLR23-FiniteCongruencesᵈ : FiniteCongruencesᵈ 𝑨
SLR23-FiniteCongruencesᵈ = certFiniteCongruencesᵈ cert certOK
```
