---
layout: default
file: "src/FLRP/Certificates/SmallLatticeReps/SLR25.lagda.md"
title: "FLRP.Certificates.SmallLatticeReps.SLR25 module (The Agda Universal Algebra Library)"
date: "2026-07-22"
author: "the agda-algebras development team (emitted by scripts/python/flrp)"
---

### A machine-checked representation: Con(B25) ≅ manuscript L25

This is the [FLRP.Certificates.SmallLatticeReps.SLR25][] module of the [Agda Universal Algebra Library][].

**This module was emitted by `scripts/python/flrp/emit_agda.py` from
`scripts/python/flrp/inputs/slr/slr25.json`.  Do not edit it by hand; rerun the emitter instead.**

`B25`/`L25` in the numbering of the DeMeo–Freese–Jipsen manuscript *Representing Finite Lattices as Congruence Lattices of Finite Algebras* (§ 6 of the 2016-06-10 draft, `docs/papers/fin-lat-rep/SmallLatticeReps.tex`).  The dictionary between the manuscript's lattice numbering and this library's names is `docs/notes/flrp-slr-naming.md`.

It re-verifies, end-to-end through the WP-6 certificate pipeline (#457), the
claim that the congruence lattice of the algebra "B25" — carrier size
5, unary operations `f`, `g`, `h`, `k` — is isomorphic to the lattice
"manuscript L25" (7 elements).  The engine's output below (normal-form
...
module FLRP.Certificates.SmallLatticeReps.SLR25 where
...
```agda
opTables : Vec (Vec (Fin 5) 5) 4
opTables = (0F ∷ 0F ∷ 2F ∷ 2F ∷ 2F ∷ [])
         ∷ (0F ∷ 1F ∷ 0F ∷ 1F ∷ 1F ∷ [])
         ∷ (1F ∷ 1F ∷ 4F ∷ 4F ∷ 4F ∷ [])
         ∷ (2F ∷ 3F ∷ 2F ∷ 3F ∷ 3F ∷ [])
         ∷ []
...
𝑨 = tablesToUnaryAlgebra 5 4 opTables

𝑭 : FiniteAlgebra 𝑨
𝑭 = tablesToUnaryAlgebra-FiniteAlgebra 5 4 opTables
...
The claimed congruence lattice "manuscript L25", presented exactly as the
...
∧-table = (0F ∷ 0F ∷ 0F ∷ 0F ∷ 0F ∷ 0F ∷ 0F ∷ [])
        ...
        ∷ (0F ∷ 0F ∷ 0F ∷ 3F ∷ 0F ∷ 3F ∷ 3F ∷ [])
        ∷ (0F ∷ 1F ∷ 2F ∷ 0F ∷ 4F ∷ 1F ∷ 4F ∷ [])
        ∷ (0F ∷ 1F ∷ 0F ∷ 3F ∷ 1F ∷ 5F ∷ 5F ∷ [])
        ∷ (0F ∷ 1F ∷ 2F ∷ 3F ∷ 4F ∷ 5F ∷ 6F ∷ [])
        ∷ []

∨-table = (0F ∷ 1F ∷ 2F ∷ 3F ∷ 4F ∷ 5F ∷ 6F ∷ [])
        ∷ (1F ∷ 1F ∷ 4F ∷ 5F ∷ 4F ∷ 5F ∷ 6F ∷ [])
        ∷ (2F ∷ 4F ∷ 2F ∷ 6F ∷ 4F ∷ 6F ∷ 6F ∷ [])
        ∷ (3F ∷ 5F ∷ 6F ∷ 3F ∷ 6F ∷ 5F ∷ 6F ∷ [])
...
cert = mkLatticeCert partsᵛ 0F prinᵛ prinTrᵛ ∧-table ∨-table joinTrᵛ
  ...
  prinTrᵛ : Vec (Vec (Trace 5 4 arOf) 5) 5
  prinTrᵛ = ( []
            ∷ ( mkMerge 0F 1F (seed 0)
              ∷ mkMerge 2F 3F (translate 3F 0F (0F ∷ []) 0)
              ∷ [] )
          ...
          ∷ ( ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (translate 3F 0F (0F ∷ []) 0)
              ∷ [] )
            ∷ []
  ...
  joinTrᵛ : Vec (Vec (Trace 5 4 arOf) 7) 7
  joinTrᵛ = ( []
            ∷ (mkMerge 4F 3F (seed 0) ∷ [])
            ∷ ( mkMerge 1F 0F (seed 0)
          ...
          ∷ ( ( mkMerge 1F 0F (seed 0)
            ...
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
              ∷ mkMerge 2F 0F (seed 2)
              ∷ mkMerge 4F 1F (seed 3)
              ∷ [] )
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
              ∷ mkMerge 4F 2F (seed 4)
              ∷ [] )
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
              ∷ mkMerge 2F 0F (seed 2)
              ∷ mkMerge 4F 1F (seed 4)
              ∷ [] )
            ∷ ( mkMerge 1F 0F (seed 0)
              ∷ mkMerge 3F 2F (seed 1)
...
```agda
SLR25-Representableᵈ : Representableᵈ (toLattice 𝑳)
SLR25-Representableᵈ = certRepresentableᵈ 𝑭 𝑺 𝑳 cert certOK certMeet

SLR25-FiniteCongruencesᵈ : FiniteCongruencesᵈ 𝑨
SLR25-FiniteCongruencesᵈ = certFiniteCongruencesᵈ cert certOK
```
