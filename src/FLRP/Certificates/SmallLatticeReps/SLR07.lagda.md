---
layout: default
file: "src/FLRP/Certificates/SmallLatticeReps/SLR07.lagda.md"
title: "FLRP.Certificates.SmallLatticeReps.SLR07 module (The Agda Universal Algebra Library)"
date: "2026-07-22"
author: "the agda-algebras development team (emitted by scripts/python/flrp)"
---

### A machine-checked representation: Con(B7) ≅ manuscript L7

This is the [FLRP.Certificates.SmallLatticeReps.SLR07][] module of the [Agda Universal Algebra Library][].

**This module was emitted by `scripts/python/flrp/emit_agda.py` from
`scripts/python/flrp/inputs/slr/slr07.json`.  Do not edit it by hand; rerun the emitter instead.**

`B7`/`L7` in the numbering of the DeMeo–Freese–Jipsen manuscript *Representing Finite Lattices as Congruence Lattices of Finite Algebras* (§ 6 of the 2016-06-10 draft, `docs/papers/fin-lat-rep/SmallLatticeReps.tex`).  The dictionary between the manuscript's lattice numbering and this library's names is `docs/notes/flrp-slr-naming.md`.

It re-verifies, end-to-end through the WP-6 certificate pipeline (#457), the
claim that the congruence lattice of the algebra "B7" — carrier size
6, unary operations `f`, `g`, `h` — is isomorphic to the lattice
"manuscript L7" (6 elements).  The engine's output below (normal-form
...
module FLRP.Certificates.SmallLatticeReps.SLR07 where
...
The claimed congruence lattice "manuscript L7", presented exactly as the
...
cert = mkLatticeCert partsᵛ 0F prinᵛ prinTrᵛ ∧-table ∨-table joinTrᵛ
  ...
  prinTrᵛ = ( []
            ...
            ∷ ( mkMerge 0F 3F (seed 0)
              ...
              ∷ mkMerge 5F 2F (translate 1F 0F (0F ∷ []) 0)
              ∷ [] )
          ...
          ∷ ( ( mkMerge 3F 0F (seed 0)
              ...
              ∷ mkMerge 2F 5F (translate 1F 0F (0F ∷ []) 0)
              ∷ [] )
...
```agda
SLR07-Representableᵈ : Representableᵈ (toLattice 𝑳)
SLR07-Representableᵈ = certRepresentableᵈ 𝑭 𝑺 𝑳 cert certOK certMeet

SLR07-FiniteCongruencesᵈ : FiniteCongruencesᵈ 𝑨
SLR07-FiniteCongruencesᵈ = certFiniteCongruencesᵈ cert certOK
```
