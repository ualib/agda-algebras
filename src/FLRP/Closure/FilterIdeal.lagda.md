...
The `Assembly`{.AgdaModule} submodule then turns a *closed finite family*
...
`ConIsoᵈ`{.AgdaFunction}` 𝑩 𝑳₀` and the `Representableᵈ`{.AgdaRecord} witness.
This lands with **no postulate and no registry assumption**, in contrast to the
Kurzweil–Netter duality route (`dual-Representableᵈ`{.AgdaFunction} of
[FLRP.Closure.Basic][]).
...
open import Data.Fin.Base                          using  ( Fin ; _↑ˡ_ ; _↑ʳ_ ; combine
                                                          ; splitAt ; remQuot )
...
Following the house pattern of [FLRP.Representable][]'s `decToFin`{.AgdaFunction},
the value of `h` is a pure function `hVal`{.AgdaFunction} of the two decision
...
```agda
  private
    ...
    hVal-const-b :  {P Q : Type 0ℓ} (a b x : 𝕌[ 𝑨 ]) (dp : Dec P) (dq : Dec Q)
...
```agda
  hMap-cong : (s : HSym) {x y : 𝕌[ 𝑨 ]} → x ≈ y → hMap s x ≈ hMap s y
  hMap-cong (a , b , u) {x} {y} x≈y = aux (α .proj₂ a b) (β .proj₂ u x) (β .proj₂ u y)
    where
    open Setoid 𝔻[ 𝑨 ] using () renaming ( refl to ≈refl )

    aux : {P : Type 0ℓ} (dp : Dec P) (dx : Dec (ConRel β u x)) (dy : Dec (ConRel β u y))
...
`𝑩`{.AgdaFunction} interprets the old symbols exactly as `𝑨`{.AgdaBound} does and
each new symbol by its `hMap`{.AgdaFunction}, on the *same* carrier setoid.  The
congruence obligation for the old symbols is discharged through the diagonal
congruence of `𝑨`{.AgdaBound} (whose compatibility field *is* the statement that
the operations respect `≈`).

```agda
  private
    -- The operations of 𝑨 respect ≈, read off the diagonal congruence.
    ≈-compat : (f : OperationSymbolsOf 𝑆) {u v : ArityOf 𝑆 f → 𝕌[ 𝑨 ]}
      → (∀ i → u i ≈ v i) → (f ^ 𝑨) u ≈ (f ^ 𝑨) v
    ≈-compat f h = lower (is-compatible (𝟘[ 𝑨 ] {0ℓ} .proj₂) f (lift ∘ h))

    -- The interpretation of the extended signature.
    interp⁺ : (o : OperationSymbolsOf 𝑆⁺) → Op (ArityOf 𝑆⁺ o) 𝕌[ 𝑨 ]
    interp⁺ (inj₁ f) = f ^ 𝑨
    interp⁺ (inj₂ s) = λ as → hMap s (as 0F)

    interp⁺-cong : (o : OperationSymbolsOf 𝑆⁺) {u v : ArityOf 𝑆⁺ o → 𝕌[ 𝑨 ]}
...
A decidable congruence of `𝑩`{.AgdaFunction} restricts to one of `𝑨`{.AgdaBound}
...
observation that the operations of `𝑨`{.AgdaBound} respect every congruence of
`𝑨`{.AgdaBound}, this is precisely the manuscript's `ops(𝑩) ⊆ λ(L₀)`.)

```agda
  hMap-compat : (s : HSym) (γ : DecCon 𝑨 0ℓ) → InFilterIdeal γ
    → {x y : 𝕌[ 𝑨 ]} → ConRel γ x y → ConRel γ (hMap s x) (hMap s y)
  ...
  hMap-compat (a , b , u) γ (inj₁ α⊆γ) {x} {y} γxy =
    ...
    aux : (dp : Dec (ConRel α a b)) (dx : Dec (ConRel β u x)) (dy : Dec (ConRel β u y))
...
```agda
  h-violation : (d : DecCon 𝑩 0ℓ) → ¬ α ⊆ᵈ restrictᵈ d → ¬ restrictᵈ d ⊆ᵈ β → ⊥
  h-violation d@((_θ_ , θcon) , _) ¬filter ¬ideal = step
    where
    ...
    αab : ConRel α a b
    αab = wα .proj₂ .proj₂ .proj₁

    ¬θab : ¬ a θ b
    ¬θab = wα .proj₂ .proj₂ .proj₂

    θuv : u θ v
    θuv = wβ .proj₂ .proj₂ .proj₁

    ¬βuv : ¬ ConRel β u v
    ¬βuv = wβ .proj₂ .proj₂ .proj₂

    β-refl : ConRel β u u
    β-refl = IsEquivalence.refl (is-equivalence (β .proj₁ .proj₂))

    -- Compatibility of the congruence with its own h operation, at (u , v).
    θ-h : ConRel d (hMap (a , b , u) u) (hMap (a , b , u) v)
    θ-h = is-compatible θcon (inj₂ (a , b , u)) (λ _ → θuv)
...
**Snow's lemma** (classification form).
Deciding the two containments over the finite carrier leaves the impossible case
to the violation step.
...
```agda
  module Assembly
    (𝑳          : FiniteLattice) (open FiniteLattice 𝑳 renaming (Carrier to L))
    (γ          : L → DecCon 𝑨 0ℓ)
    (γ-mem      : ∀ k → InFilterIdeal (γ k))
    (classify   : (e : DecCon 𝑨 0ℓ) → InFilterIdeal e
                → Σ[ k ∈ L ] proj₁ e ≑ proj₁ (γ k))
    (γ-mono     : ∀ k l → γ k ⊆ᵈ γ l → k ∧ l ≡ k)
    (γ-reflect  : ∀ k l → k ∧ l ≡ k → γ k ⊆ᵈ γ l)
    where
...
enumeration of a `FiniteAlgebra`{.AgdaRecord} does not suffice).  The enumeration
of the symbol sum and of the triples is assembled from the standard
`Fin`{.AgdaDatatype} splitting and pairing combinators.
...
```agda
  private
    ...
    pairEnum {m = m} {n = n} eA eB k =
      eA (remQuot {m} n k .proj₁) , eB (remQuot {m} n k .proj₂)

    pairEnum-sur : {A B : Type 0ℓ} {m n : ℕ} (eA : Fin m → A) (eB : Fin n → B)
    ...
    sumEnum-sur :  {A B : Type 0ℓ} {m n : ℕ} (eA : Fin m → A) (eB : Fin n → B)
      → (∀ a → ∃[ i ] eA i ≡ a) → (∀ b → ∃[ j ] eB j ≡ b)
  ...
  𝑆⁺-FiniteSignature : FiniteSignature 𝑆
    → (n : ℕ) (e : Fin n → 𝕌[ 𝑨 ]) (e-sur : ∀ x → ∃[ i ] e i ≡ x) → FiniteSignature 𝑆⁺
  𝑆⁺-FiniteSignature 𝑺 n e e-sur .opCard = 𝑺 .opCard + n * (n * n)
  𝑆⁺-FiniteSignature 𝑺 n e e-sur .opEnum = sumEnum (𝑺 .opEnum) (pairEnum e (pairEnum e e))
...
[^1]: See `docs/papers/fin-lat-rep/SmallLatticeReps.tex` § "Union of a filter and ideal" for the informal proof.

[^2]: The representability notion at play here is the one that appears in the
      closure test `Inv(M) = L` of the preserving monoid of `scripts/python/flrp/eqsearch.py`.
