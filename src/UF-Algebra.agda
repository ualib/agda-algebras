...
open import UF-Prelude using (Universe; 𝓤; 𝓤₀;𝓥; 𝓦; 𝓣; _⁺; _̇;_⊔_; universe-of; id; 𝑖𝑑; _∘_; _,_; Σ; -Σ; pr₁; pr₂; Π; -Π; domain; _×_; _≡_; refl; _∼_; transport; _≡⟨_⟩_; _∎; ap; _∙_; _⁻¹; _⇔_; _iff_; lr-implication; rl-implication; 𝟘; 𝟚)

open import UF-Equality using (refl-left; ap-id; _◁_; singleton-type'; singleton-types'-are-singletons; _≃_;  id-≃; is-equiv; id-is-equiv; Σ-≡-≃; Σ-cong; ≃-sym; _≃⟨_⟩_; _■; ⌜_⌝; ⌜⌝-is-equiv; ∘-is-equiv; inverse; to-×-≡; ap-pr₁-to-×-≡; ap-pr₂-to-×-≡; inverses-are-sections; fiber; fiber-point; fiber-identification; Σ-flip)
...
{-Following Martin Escardo (op. cit.), we consider mathematical structures specified by a *structure function*
    `S : 𝓤 ̇ → 𝓥 ̇`
  ...
  Escardo describes the identity type `Id (Σ S) A B`, in "favourable circumstances", in terms of equivalences of the underlying
...
⟦ _ , s ⟧ = s  -- alias for `structure`

{-The "favorable circumstances" according to Escardo are the following:

      * `ι : (A B : Σ S) → ⟨ A ⟩ ≃ ⟨ B ⟩ → 𝓦 ̇` describes favorable equivalences (which turn out to be the homs)
      * `ρ : (A : Σ S) → ι A A (id-≃ ⟨ A ⟩)`          stipulates that all identity equivalences are homomorphisms.

   Escardo also requires that two structures on the same type making the identity equivalence a homomorphism must be
   identified in a canonical way.   Accordingly, we have the canonical map `s ≡ t  → ι (X , s) (X , t) (id-≃ X)`, defined by induction
   on identifications by `refl s ↦ ρ (X , s)`, and this map must be an equivalence for all `X : 𝓤` and `s t : S X`.
-}
...
--These data are captured by a type which represents a "standard notion of structure:"
...
{- An inhabitant of `SNS S 𝓦` is thus a triple, ( ι , ρ , θ ), where

    ι ꞉ ( A B : Σ S ) → ( ⟨ A ⟩ ≃ ⟨ B ⟩ → 𝓦 ̇ )    describes equivalences (which turn out to be the homomorphisms)
    ρ ꞉ ( ( A : Σ S ) → ι A A ( id-≃ ⟨ A ⟩ ) )           stipulates that all identity equivalences are homomorphisms, and
    θ : ( s t : S X ) → is-equiv ( canonical-map ι ρ s t ) )     says that the canonical-map is an equivalence.
   ...
   fibers translates essentially to having only singleton kernel blocks (or being "one-to-one") -}

--Escardo uses the label `homomorphic` for the first projection of his SNS.
...
A ≃[ σ ] B = Σ f ꞉ ( ⟨ A ⟩ → ⟨ B ⟩ ) ,     -- ∃ map from carrier ⟨ A ⟩ of A to carrier ⟨ B ⟩ of B;
                      Σ feq ꞉ is-equiv f ,                     -- it has singleton fibers, and so
                           homomorphic σ A B ( f , feq )        -- yields a homomorphism from A to B.
--infix 20 _≈[_]_
...
--With this, Escardo proves a number of characterizations of identity on `Σ S`.

--First, a homomorphism lemma:
...
--Second, assuming univalence of 𝓤, an identification between the equivalences `≡` and `≃[ σ ]`:
...
--Escardo observes that the above equivalence is pointwise equal to `Id→homEq`, so `Id→homEq` is itself an equivalence:
...
Id-homEq-is-equiv 𝓤★ {S} σ A B = γ
   where
    ...
    γ = equivs-closed-under-∼ ( ⌜⌝-is-equiv (characterization-of-≡ 𝓤★ σ A B ) ) ( h A B )

--Finally, Escardo gives a characterization of the canonical map and of when it is an equivalence, applying Yoneda.
module _ { S : 𝓤 ̇ → 𝓥 ̇}
             ( ι : ( A B : Σ S ) → ⟨ A ⟩ ≃ ⟨ B ⟩ → 𝓦 ̇ )
             ( ρ : ( ( A : Σ S ) → ι A A ( id-≃ ⟨ A ⟩ ) ) )
             {X : 𝓤 ̇}  where
  ...
  canonical-map-charac : ( s t : S X ) ( s≡t : s ≡ t )
   →   ( canonical-map ι ρ s t s≡t )
            ≡  ( transport ( λ - → ι ( X , s ) ( X , - ) ( id-≃ X ) ) s≡t ( ρ ( X , s ) ) )
