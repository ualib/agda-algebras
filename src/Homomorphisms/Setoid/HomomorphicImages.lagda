...
This is the [Homomorphisms.Setoid.HomomorphicImages][] module of the [Agda Universal Algebra Library][].
...
open import Overture.Setoid.Preliminaries               using ( lift∼lower )
open import Overture.Setoid.Inverses                    using ( Ran ; _range ; _preimage ; _image ; Inv )
                                                        using ( _preimage≈image ; InvIsInverseʳ ; Image_∋_ )
open import Overture.Setoid.Surjective                  using ( IsSurjective ; ∘-IsSurjective )
open import Algebras.Setoid.Basic              {𝑆 = 𝑆}  using ( Algebra ; ov ; _̂_ ; ⟦_⟧ ; Lift-Algˡ )
                                                        using ( Lift-Alg ; 𝕌[_] )
open import Homomorphisms.Setoid.Basic         {𝑆 = 𝑆}  using ( hom ; IsHom )
open import Homomorphisms.Setoid.Isomorphisms  {𝑆 = 𝑆}  using ( _≅_ ; Lift-≅ )
open import Homomorphisms.Setoid.Properties    {𝑆 = 𝑆}  using ( Lift-homˡ ; ToLiftˡ ; lift-hom-lemma )
                                                        using ( 𝒾𝒹 ; ∘-hom )
...
IsHomImageOfClass : {𝒦 : Pred (Algebra α ρᵃ)(lsuc α)} → Algebra α ρᵃ → Type (ov (α ⊔ ρᵃ))
...
<span style="float:left;">[← Homomorphisms.Setoid.Isomorphisms](Homomorphisms.Setoid.Isomorphisms.html)</span>
