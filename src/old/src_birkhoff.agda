...
-- Obs 2.0 vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
-- composing homs gives a hom
--
comp : {𝑨 𝑩 𝑪 : algebra S} -> hom 𝑨 𝑩 -> hom 𝑩 𝑪 -> hom 𝑨 𝑪
comp (mkhom f α) (mkhom g β) =
  mkhom (g ∘ f) (λ x → trans (cong g (α x)) (β _))
--
-- End Obs 2.0 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Eq-closed : ∀{𝓸 : S 𝓕}{𝑨 𝑩 : algebra S}
  ->        (f g : hom 𝑨 𝑩)
  ->        (args : ℕ → ⟦ 𝑨 ⟧ᵤ)
  ->        (∀ i -> (args i) ∈ (E-hom f g))
         -----------------------------------------------------
   ->    ⟦ f ⟧ₕ ((𝑨 ⟦ 𝓸 ⟧) args) ≡ ⟦ g ⟧ₕ ((𝑨 ⟦ 𝓸 ⟧) args)
Eq-closed {𝓸} {𝑨} {𝑩} f g args p = 
  begin
     ⟦ f ⟧ₕ ((𝑨 ⟦ 𝓸 ⟧) args)
  ≡⟨ homo f args ⟩
    (𝑩 ⟦ 𝓸 ⟧) (λ i ->  ⟦ f ⟧ₕ (args i))
  ≡⟨  cong ((𝑩 ⟦_⟧)_) (∀-extensionality λ x → p x) ⟩
    (𝑩 ⟦ 𝓸 ⟧) (λ i -> ⟦ g ⟧ₕ (args i))  
  ≡⟨ sym (homo g args) ⟩
    ⟦ g ⟧ₕ ((𝑨 ⟦ 𝓸 ⟧) args)   
  ∎
...
--
--Equalizer E(f, g) of homs f, g : hom 𝑨 𝑩 is a subuniverse of 𝑨.
eqIsSub : {𝑨 𝑩 : algebra S} -> hom 𝑨 𝑩 → hom 𝑨 𝑩 → subuniverse 𝑨
eqIsSub f g = mksub (E-hom f g) (Eq-closed f g)
--
-- End Obs 2.1 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
