...
+  [FLRP.Hunt][]: the RP-3 **constraint dossier** for the hunt for an
   ...
   representation of the one-element chain, the refutation of the unrestricted
   pair statement and its repaired form `PairQuestion`, the closure of the
   two-element corner of the RP-4 reduction (outright on chains, by padding
   under statement (C) otherwise), the theorem that families of two-element
   chains have inhabited intersection, and the wreath-richness of
   two-element-chain classes; the survey note is `docs/notes/flrp-rp3-hunt.md`.
