...
+  `eqsearch.py` — the search side (issue #486), converse in direction to the emitters: given a target lattice, find its sublattice copies in `Eq(n)` with bounds at the diagonal and the total relation, classify them up to relabeling, and run the Snow closure test on each class; a closed class yields the witness algebra `⟨X, M⟩` and a ready-made claim file for the emitter.  `--group-rep` (issue #494) restricts the sweep to uniform copies — see the uniform-restriction note below for the weakened negative-verdict semantics.
...
```sh
python3 scripts/python/flrp/eqsearch.py TARGET.json N [--fast] [--group-rep] [--json REPORT.json]
...
**The fast backend**.  `--fast` routes the survey through `eqfast.py`: numpy-vectorized meet/join table construction, mask-arithmetic candidate filtering over the same assignment plan as the pure search, and per-class vectorized orbit generation, with closure verdicts delegated to the pure implementation so the backends cannot drift where it matters.  The reports are byte-identical to the pure engine's (pinned by `test_eqfast.py`).  numpy is deliberately not a dependency of the pure tools — `--fast` degrades to a clear error without it and its tests skip cleanly — but the nix dev shell ships it (`flake.nix`), so under `nix develop` the fast path works out of the box; outside the shell, install numpy any convenient way (for instance `python3 -m venv .venv && .venv/bin/pip install numpy`).  Measured scale on one core: `Eq(6)` in ~2 s and `Eq(7)` in ~30 s (versus about five minutes for `Eq(7)` with the pure engine), while the committed `Eq(8)` run took about three hours: the generic height-ordered assignment plan defers join constraints, so intermediate prefix counts balloon at eight points.  A constraint-density-guided assignment order (place an element as soon as it is the meet or join of placed ones) is the known fix — a schedule-specific engine with exactly that order ran `Eq(8)` in minutes — and is follow-up work on #486, as is the blocked-table build that the *unrestricted* `Eq(9)` (`Bell(9) = 21,147`, ~1.8 GB of tables at `int16`) needs; under `--group-rep` the table problem disappears instead (see the next note).

**The uniform restriction (`--group-rep`)**.  `--group-rep` (issue #494, on either engine) restricts the sweep to copies all of whose members are **uniform** partitions — every block of one size `k`, necessarily dividing `n` — enumerated per divisor directly, never through the full Bell(`n`) census, in the engines' canonical order.  This mechanizes remark iv of the SmallLatticeReps closure-algorithm discussion (`SmallLatticeReps.tex` § 6.1): the congruences of a transitive permutation group action are its systems of imprimitivity, whose blocks are cosets of intermediate subgroups and hence all of equal size, so every congruence lattice of a transitive `G`-set occurs among uniform copies.  (Naming: the campaign's target `L7` is this library's numbering; the manuscript calls the same lattice `L10` — dictionary in `docs/notes/flrp-slr-naming.md`.)  Positive verdicts keep exactly their unrestricted meaning, because the Snow closure test is unchanged: the preserving monoid `M` still ranges over *all* unary maps and `Inv(M)` over *all* of `Eq(n)`, so a closed uniform class yields the same honest witness algebra `⟨X, M⟩` and claim file as an unrestricted find.  **Negative verdicts weaken**: "no closed uniform class on `n` points" rules out algebras whose congruence lattices consist of uniform partitions — in particular every transitive group action — rather than all algebras; for library `L7` at the minimal frontier that is exactly the statement needed, since the manuscript's § 5 analysis shows a minimal representation of `L10` = library `L7`, if one exists, must come from a transitive permutation group, and the machine-checked closure obstructions through eight points put that minimal frontier at nine points or beyond.  A restricted report records `"restriction": "uniform"` between `points` and `copies`, so a uniform census can never be mistaken for a full one; unrestricted reports are byte-for-byte unchanged.  The pool is vanishingly small — 25, 0, 140, 280, and 1,071 nontrivial uniform partitions for `n` = 6, 7, 8, 9, 10, against Bell numbers 203 through 115,975; prime `n` has none, matching the group fact that a transitive action of prime degree is primitive — so the sweeps the unrestricted engine cannot touch at all come into range with no large tables anywhere: the fast backend swaps the dense `n^n` code table (~1.5 GB at `n = 9`, ~40 GB at `n = 10`) for binary search over the sorted pool codes.  Measured 2026-07-24, one core: `Eq(9)` in 0.65 s fast / 1.87 s pure, and `Eq(10)` in 24 m fast / **7 m pure** — at a pool this narrow the constant per-prefix cost of mask arithmetic outweighs the vectorization, so the pure engine wins at ten points; the verdicts are byte-identical either way, and the committed `Eq(9)`/`Eq(10)` uniform reports (`out/l7_eq9_uniform_report.json`, `out/l7_eq10_uniform_report.json` — both empty censuses, see `docs/notes/flrp-l7-eq6.md` § 8) are pinned by `make flrp-test`, the ten-point one behind `FLRP_EQSEARCH_SLOW=1`.

**Scaling `--group-rep` to twelve points (issue #499)**.  The `Eq(12)` uniform pool has 32,032 nontrivial members (shapes `2⁶`, `3⁴`, `4³`, `6²`), two orders of magnitude past the sizes the #494 engines were built for, and three costs blow up there: the eager pool² meet/join tables (~4.1 GB at `int16`), the classifier's materialized `12! ≈ 4.8 × 10⁸` per-class relabelings, and the closure universe (`Bell(12) = 4,213,597` partitions, ~0.8 GB as tuples).  Issue #499 brings each within reach without moving any report.  Classification is by **orbit–stabilizer**: distinct relation sets are bucketed by a relabeling-invariant key, split into orbits by an explicit setwise-isomorphism test, and each orbit's size is reported as `n!/|stab|` from a stabilizer backtrack, guarded by the conservation check `Σ n!/|stab| = #distinct relation sets`; `classify` dispatches to the materialized reference while `n! ≤ 10!`, so every committed census is byte-for-byte unchanged.  Meet and join are computed **on the fly and memoized** (`LazyUniformTables` / `FastLazyUniformTables`, chosen once the pool exceeds `_EAGER_POOL_LIMIT`) rather than tabulated.  The closure universe is built lazily — an empty census never pays for it — and the fast backend packs it as a 50.6 MB `int8` matrix filtered by a vectorized invariance mask, the pure backend as tuples.  All the new paths are pinned byte-identical to the eager/materialized ones on `M3`, `L7`, and the uniform censuses in `make flrp-test`.

**The `Eq(12)` sweep and its verdict (`eq12_uniform_sweep.py`, issue #499)**.  Even with those costs tamed, the *generic* height-ordered search still balloons at twelve — element 1 alone ranges over all 32,032 uniform partitions with ~6,700 `Δ`-meeting partners each, a projected 584 hours.  `eq12_uniform_sweep.py` runs the census to a verdict in about four minutes with a constraint-density, symmetry-broken search specialised to `L7`: it anchors on the coatom `(1,1)`, whose block size is a composite proper divisor of twelve (only 4 or 6), fixed to two canonical representatives since `S₁₂` is transitive on the uniform partitions of a block size, and derives the atoms and the second coatom as tightly constrained refinements.  Completeness is certified — the per-class stabilizer counts `Σ |Stab(c_k)|/|stab|` must reproduce the exhaustively counted canonical-coatom copies, so no class hides — and classification counting and every closure verdict are delegated to the shared engine, so the census means exactly what the smaller ones do.  The verdict: `Eq(12)` holds 3,353,011,200 uniform copies of `L7` in **15 relabeling classes, none closed**, so no algebra on twelve points has `Con ≅ L7`; with the antichain uniform pools at 13, 14, 15 the minimal representation, if one exists, has at least sixteen elements (`docs/notes/flrp-l7-eq6.md` § 10).  Committed as `out/l7_eq12_uniform_report.json`, re-derived behind `FLRP_EQSEARCH_SLOW=1`.  Deriving the constraint-density order automatically for an arbitrary target is the #486 follow-up.
...
`test_eqsearch.py` covers the search side: the partition kernel cross-validated against brute force (the backtracking monoid enumeration versus a full `n^n` scan), the `M3`-on-4-points census with its closed matchings class flowing all the way to a checked certificate, and the pinned `L7` session results.  The `Eq(7)` sweep (about five minutes) runs only with `FLRP_EQSEARCH_SLOW=1`.  For the uniform restriction it pins the pool enumerator (the #494 count table for `n` = 6–10, and the per-divisor enumeration cross-checked order-for-order against the Bell filter for `n ≤ 8`), the pool tables' out-of-pool sentinel and full-`Eq(n)` closure universe, and **filter parity** — the `--group-rep` census must equal the all-uniform subset of the unrestricted census, class report for class report, checked for `M3` on 4 and 6 points and for `L7` on 6 (pinned *empty*: both known classes contain the non-uniform grid element `|0,2|3,4|`), with the prime degeneration at 7 (a bounds-only pool and an empty sweep).  For the #499 scaling it pins the orbit–stabilizer classifier byte-identical to the materialized reference (`M3`/`Eq(4)`, `M3`/`Eq(6)`, `L7`/`Eq(6)`, and behind `FLRP_EQSEARCH_SLOW=1` `L7`/`Eq(7)`), the on-the-fly `LazyUniformTables` against the eager pool tables and a forced-lazy survey report, the `Eq(12)` pool counts (32,032 = 10,395 + 15,400 + 5,775 + 462), and the frontier's divisor structure at 13, 14, 15.
...
`test_eqfast.py` pins the fast backend to the pure one — equal tables, byte-identical survey reports on `M3`/`Eq(4)` and `L7`/`Eq(6)`, the `Eq(7)` census in seconds — and, behind `FLRP_EQSEARCH_SLOW=1`, re-runs the `Eq(8)` sweep and checks its output against the committed `out/l7_eq8_report.json` byte for byte.  The uniform restriction is held to the same parity standard in the default suite: pool tables with their sentinels equal the pure ones (`n` = 6, 8), and `--group-rep` reports are byte-identical through `Eq(9)`; since both engines now share the orbit–stabilizer `classify` (issue #499), the fast backend adds parity of its own on-the-fly rows and its vectorized closure filter (`FastLazyUniformTables` versus the eager tables and the pure `invariant_partitions`, plus a forced-lazy survey report).  Behind `FLRP_EQSEARCH_SLOW=1` the `Eq(8)` filter parity checks the `--group-rep` classes against the all-uniform subset of the committed report (the committed report is the fixture — the unrestricted sweep is never re-run).  All of it skips cleanly when numpy is absent.
