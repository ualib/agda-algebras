...
open import prelude using (global-dfunext; dfunext; 𝓤ω; _⨉_)
...
-- AlgPred : (𝓤 𝓦 : Universe) → (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) ̇ → (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺ ⊔ 𝓦) ⁺ ̇
-- AlgPred 𝓤 𝓦 𝑨 = 𝑨 → (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺ ⊔ 𝓦) ̇

-- We need a type that allows for different universe levels.

hclosed : (ℒ𝒦 : (𝓤 : Universe) → Pred (Algebra 𝓤 𝑆) 𝓤) → 𝓤ω
hclosed ℒ𝒦 = ∀(𝓧 : Universe)(𝑨 : Algebra 𝓧 𝑆)
 →            𝑨 ∈ ℒ𝒦 𝓧
 →            ((𝑩 , _ , _) : HomImagesOf 𝑨)
            -----------------------------------
 →            𝑩 ∈ ℒ𝒦 𝓧

sclosed : (ℒ𝒦 : (𝓤 : Universe) → Pred (Algebra 𝓤 𝑆) 𝓤) → 𝓤ω
sclosed ℒ𝒦 = ∀(𝓧 : Universe)(𝑨 : Algebra 𝓧 𝑆)
 →            𝑨 ∈ (ℒ𝒦 𝓧) → (sa : SubalgebrasOf 𝑨) → ∣ sa ∣ ∈ (ℒ𝒦 𝓧)

pclosed : (ℒ𝒦 : (𝓤 : Universe) → Pred (Algebra 𝓤 𝑆) 𝓤) → 𝓤ω
pclosed ℒ𝒦 = ∀(𝓧 𝓘 : Universe)(I : 𝓘 ⁺ ̇ )(𝒜 : I → Algebra 𝓧 𝑆) → (∀ i → 𝒜 i ∈ (ℒ𝒦 𝓧))
  → (⨅ 𝒜) ∈ (ℒ𝒦 ((𝓧 ⊔ 𝓘 ⁺)))

vclosed : (ℒ𝒦 : (𝓤 : Universe) → Pred (Algebra 𝓤 𝑆) 𝓤) → 𝓤ω
vclosed ℒ𝒦 = (hclosed ℒ𝒦 ⨉ sclosed ℒ𝒦) ⨉ pclosed ℒ𝒦
...
module _
 ...
 𝔽∈ℒ𝒦vclo : {𝓤 𝓦 : Universe}{X : 𝓤 ̇}
             (ℒ𝒦 : (𝓧 : Universe) → Pred (Algebra 𝓧 𝑆) 𝓧)
  →          vclosed ℒ𝒦
  →          𝔽{𝓤}{𝓦}{X} ∈ ℒ𝒦 ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) ⁺)

 𝔽∈ℒ𝒦vclo {𝓤}{𝓦}{X} ℒ𝒦 ℒ𝒦vclo = {!γ!}
  where
   ΣP : (𝓧 : Universe) → 𝓞 ⊔ 𝓥 ⊔ 𝓧 ⁺ ̇
   ΣP 𝓧 = Σ 𝑨 ꞉ (Algebra 𝓧 𝑆) , 𝑨 ∈ ℒ𝒦 𝓧

   𝒜ΣP : (𝓧 : Universe) → ΣP 𝓧 → Algebra 𝓧 𝑆
   𝒜ΣP 𝓧 i = ∣ i ∣

   𝒜ΣP' : (𝓧 : Universe) → Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓧 ⁺) 𝑆
   𝒜ΣP' 𝓧 = ⨅ (𝒜ΣP 𝓧)

   ⨅𝒜ΣP : Algebra ((𝓞 ⊔ 𝓥 ⊔ 𝓧) ⁺) 𝑆  -- 𝓤ω  -- 
   ⨅𝒜ΣP = ? -- ⨅ 𝒜ΣP'

   -- 𝑨 : Algebra ((𝓞 ⊔ 𝓥 ⊔ 𝓤) ⁺ ⁺) 𝑆
   -- 𝑨 = ⨅𝒜ΣP {𝓘 = 𝓤}

   ⨅𝒦∈vclo : {𝓘 : Universe}{I : (𝓞 ⊔ 𝓥 ⊔ 𝓘) ⁺ ̇}{𝒜 : I → Algebra ((𝓞 ⊔ 𝓥 ⊔ 𝓘)) 𝑆}
              (p : ∀ (i : I) → 𝒜 i ∈ ℒ𝒦 (𝓞 ⊔ 𝓥 ⊔ 𝓘))
     →        ⨅ 𝒜 ∈ ℒ𝒦 ((𝓞 ⊔ 𝓥 ⊔ 𝓘) ⁺)
   ⨅𝒦∈vclo p = {!!} -- vprod ?
 ...
 𝔽∈vclo {𝓤}{𝓦}{X} = γ
  where
   ...
   𝑨∈vclo : 𝑨 ∈ vclo{(𝓞 ⊔ 𝓥 ⊔ 𝓤) ⁺}(ℒ𝒦 ((𝓞 ⊔ 𝓥 ⊔ 𝓤) ⁺))
   𝑨∈vclo = {!⨅𝒦∈vclo{𝓘 = 𝓤}{𝒜 = 𝒜ΣP{𝓘 = 𝓤}} ?!}
