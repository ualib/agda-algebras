...
-- 𝕀→Alg : {𝓠 : Universe}{𝒦 : Pred (Algebra 𝓠 𝑆) (𝓞 ⊔ 𝓥 ⊔ 𝓠 ⁺)}
--  →          𝕀{𝓠}{(𝓞 ⊔ 𝓥 ⊔ 𝓠 ⁺)} 𝒦 → Algebra 𝓠 𝑆
-- 𝕀→Alg (_ , 𝒜 , _) = ⨅ 𝒜

-- ⨅Class : {𝓠 : Universe} → Pred (Algebra 𝓠 𝑆) (𝓞 ⊔ 𝓥 ⊔ 𝓠 ⁺) → Algebra (𝓞 ⊔ 𝓥  ⊔ 𝓠 ⁺) 𝑆
-- ⨅Class {𝓠} 𝒦 = ⨅ (𝕀→Alg{𝓠}{𝒦})

-- --Example
-- ⨅SClo : {𝓠 : Universe} → Pred (Algebra 𝓠 𝑆) (𝓞 ⊔ 𝓥 ⊔ 𝓠 ⁺)  → Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓠 ⁺) 𝑆
-- ⨅SClo{𝓠} 𝒦 = ⨅Class{𝓠} (SClo 𝒦)

𝕀 : {𝓤 : Universe} → Pred (Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆) ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺) → (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺ ̇
𝕀 {𝓤} 𝒦 = Σ I ꞉ 𝓤 ̇ , Σ 𝒜 ꞉ (I → Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆) , ∀ i → 𝒜 i ∈ 𝒦

𝕀→Alg : {𝓤 : Universe}{𝒦 : Pred (Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆) ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺)}
 →          𝕀{𝓤} 𝒦 → Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆
𝕀→Alg (_ , 𝒜 , _) = ⨅ 𝒜
...
-- ⨅SClo : {𝓤 : Universe} → Pred (Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆) ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺) → Algebra ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺) 𝑆
-- ⨅SClo{𝓤} 𝒦 = ⨅Class{𝓤 = (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)}{𝓣 = (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺} (SClo 𝒦)
 -- → hfunext 𝓤 𝓤 →
-- ⨅Sclo∈SP : {𝓤 : Universe}{𝒦 : Pred (Algebra (𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺) 𝑆) ((𝓞 ⊔ 𝓥 ⊔ 𝓤 ⁺)⁺)}
--            -----------------------------
--  →         (⨅SClo{𝓤} 𝒦) ∈ PClo (SClo 𝒦)
