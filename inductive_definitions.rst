...
The clone :math:`\mathrm{Clo}(F^𝔸)` is associated with the algebra :math:`𝔸` only insofar as the former is constructed out of the basic operations of 𝔸 and the set :math:`A` on which those operations are defined.  But all we really need is a set :math:`A` with some collection :math:`F` of operations defined on :math:`A` and we can consider the clone generated by :math:`F`, which we denote by :math:`\mathrm{Clo}(F)`.

We seek a "bottom-up," inductive description of the members of :math:`\mathrm{Clo}(F)`.  By thinking of the clone itself as a kind of algebra, a description analogous to :numref:`thm-1-14` ought to be possible.  In fact, since function composition is associative, a slightly slicker formulation is available.
...
.. proof:observation::
   ...
   .. container:: toggle
      ...
      Let :math:`F̄ = \bigcup_n F_n`. It is easy to argue by induction that every :math:`F_n` is a subset of :math:`\mathrm{Clo}(F)`. Thus, :math:`F ⊆ \mathrm{Clo}(F)`.
   ...
   Then :math:`\mathrm{Clo}^{A}(F) = ⋃_n F_n`.
   ...
   .. container:: toggle
      ...
      Let :math:`̄F = ⋃_n F_n`. It is easy to argue by induction that every :math:`F_n` is a subset of :math:`\mathrm{Clo}(F)`. Thus, :math:`F ⊆ \mathrm{Clo}(F)`.
    
      For the converse, we must show that :math:`F̄` is a clone containing :math:`F`.
    
      Since :math:`F_0 ⊆ F̄`, we see that :math:`F̄` contains the projection operations. 

      Also, for every operation :math:`f`, we have :math:`f[π^k_1,\dots, π^k_k] ∈ F_1 ⊆ F̄`, where :math:`k:= ρ f`.
 
      We are reduced to showing that :math:`F̄` is closed under generalized composition. This follows from the following claim.
 
      **Claim**. If :math:`f ∈ F_n` and :math:`g_1, \dots, g_{ρ f} ∈ F_m` are all of the same rank, then :math:`f[g_1, \dots, g_{ρ f}] \in F_{n+m}`.
 
      We prove the claim by induction on :math:`n`.
      
      If :math:`n = 0` then :math:`f` is a projection, so :math:`f[g_1, \dots, g_{ρ f}] = g_i ∈ F_{0+m}` for some :math:`i ≤ {ρ f}`.

      So assume the claim holds for :math:`n` and that :math:`f ∈ F_{n+1} - F_n`.
      
      From the definition, there is an operation :math:`f_i ∈ F` of rank :math:`t`, and there are opereations :math:`h_1, \dots, h_t \in F_n`, such that :math:`f = f_1[h_1, \dots, h_t]`. Note that the rank of each :math:`h_i` must be equal to the rank of :math:`f`, namely :math:`ρ f`.
      
      By the induction hypothesis, for each :math:`i ≤ k`, :math:`h_i' = h_i[g_1, \dots, g_k] \in F_{n+m}`. Applying the definition, :math:`f_1[h_1',\dots, h'_t] ∈ F_{n+m+1} = F_{(n+1)+m}`. Since 
      
      .. math:: f_1[h_1',\dots, h'_t]= f_1[h_1[\mathbf g], \dots, h_t[\mathbf g]] = f[\mathbf g].
