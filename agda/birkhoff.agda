...
module _
 ...
 identities-are-compatible-with-homs : (p q : Term{X = X})
  →                𝒦 ⊧ p ≋ q
       ----------------------------------------------------
  →     ∀ A KA h → ∣ h ∣ ∘ (p ̇ (𝔉{X = X})) ≡ ∣ h ∣ ∘ (q ̇ 𝔉)
 -- Here, the inferred types are
 -- A : Algebra 𝓤 S, KA : 𝒦 A, h : hom (𝔉{X = X}) A
 ...
 identities-are-compatible-with-homs p q 𝒦⊧p≋q A KA h = γ
  where
   ...
   hpa≡hqa : ∀(𝒂 : X → ∣ 𝔉 ∣)
    →        ∣ h ∣ ((p ̇ 𝔉) 𝒂) ≡ ∣ h ∣ ((q ̇ 𝔉) 𝒂)
   hpa≡hqa 𝒂 =
    ∣ h ∣ ((p ̇ 𝔉) 𝒂)  ≡⟨ comm-hom-term' gfe 𝔉 A h p 𝒂 ⟩
    (p ̇ A)(∣ h ∣ ∘ 𝒂) ≡⟨ pAh≡qAh 𝒂 ⟩
    (q ̇ A)(∣ h ∣ ∘ 𝒂) ≡⟨ (comm-hom-term' gfe 𝔉 A h q 𝒂)⁻¹ ⟩
    ∣ h ∣ ((q ̇ 𝔉) 𝒂)  ∎
 ...
 homs-are-compatible-with-identities : (p q : Term)
  →    (∀ A KA h  →  ∣ h ∣ ∘ (p ̇ 𝔉) ≡ ∣ h ∣ ∘ (q ̇ 𝔉))
       -----------------------------------------------
  →                𝒦 ⊧ p ≋ q
 --inferred types: A : Algebra 𝓤 S, KA : A ∈ 𝒦, h : hom 𝔉 A
  ...
  where
   ...
   γ = gfe λ 𝒂 →
    (p ̇ A) 𝒂
      ≡⟨ refl _ ⟩
    (p ̇ A)(∣ h 𝒂 ∣ ∘ generator)
      ≡⟨(comm-hom-term' gfe 𝔉 A (h 𝒂) p generator)⁻¹ ⟩
    (∣ h 𝒂 ∣ ∘ (p ̇ 𝔉)) generator
      ≡⟨ ap (λ - → - generator) (all-hp≡hq A KA (h 𝒂)) ⟩
    (∣ h 𝒂 ∣ ∘ (q ̇ 𝔉)) generator
      ≡⟨ (comm-hom-term' gfe 𝔉 A (h 𝒂) q generator) ⟩
    (q ̇ A)(∣ h 𝒂 ∣ ∘ generator)
      ≡⟨ refl _ ⟩
    (q ̇ A) 𝒂
      ∎
 ...
 compatibility-of-identities-and-homs : (p q : Term)
  →  (𝒦 ⊧ p ≋ q)
      ⇔ (∀ A ka hh → ∣ hh ∣ ∘ (p ̇ 𝔉) ≡ ∣ hh ∣ ∘ (q ̇ 𝔉))
 --inferred types: A : algebra 𝓤 s, ka : A ∈ 𝒦, hh : hom 𝔉 A.
...
birkhoff 𝒦 A g eg A∈ModThV = γ
 ...
 --By :numref:`Obs %s <obs 9>`, ℎ₀ extends to an epimorphism ℎ : 𝔉(𝑌) → A`.
 ...
 --Let u, v ∈ 𝔉(Y) and assume g(u) = g(v).
 --Since 𝔉(Y) is generated by 𝑌, there are terms 𝑝, 𝑞 ∈ 𝔉(Y) and 𝒚 such that u = p^𝔉(𝒚)
 --and v = q^𝔉(𝒚). Therefore, p^{𝔽_𝒲} 𝒚 = g(u) = g(v) = q^{𝔽_𝒲} 𝒚.
