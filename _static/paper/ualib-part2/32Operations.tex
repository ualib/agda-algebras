...
\end{code}
\ccpad
It turns out that the intepretation of a term is the same as the \af{free-lift}
...
\end{code}
\ccpad
To conclude the \ualibTerms module, we prove that every term is compatible with every congruence relation. That is, if \ab{t}~\as :~\ad{Term}~\ab X and \ab{θ}~\as :~\af{Con}~\ab 𝑨, then \ab{a}~\ab θ~\ab b~\as →~\ab t(\ab a)~\ab θ~\ab t(\ab b).
\ccpad
\begin{code}%
