...
x, the free lift of \ab h agrees with \ab h.  For the inductive step, when the given term has the form \aic{node}
...
\af{funext}~\ab 𝓥~\ab 𝓤 (that is, \emph{function extensionality} at universe levels
...
\end{code}
\ccpad
In the \ualibVarieties module we'll use \af{lift-hom} and \af{lift-of-epi-is-epi} to construct such epimorphisms.
