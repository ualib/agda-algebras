\documentclass[a4paper,USenglish,cleveref,autoref,thm-restate,anonymous]{lipics-v2019}
...
%% \affiliation{% \institution{\href{https://www.cs.cmu.edu/}{}\city{Pittsburgh}\state{PA}%\author{\href{http://www.cs.cmu.edu/~ssomayya/}{Siva Somayyajula}}
%% \institution{\href{https://prl-prg.github.io/}\streetaddress{Thákurova 9}\city{Prague 6}  \state{Czech Republic} \postcode{160 00} 
...
%% \affiliation{% \\
%%   \institution{\href{https://www.mff.cuni.cz/en/ka}{Department of Algebra}\\Charles University} %\\ Katedra Algebry, Univerzita Karlova\\%%   \streetaddress{Sokolovska 83}
%% 186 00 Praha, \v{C}esk\'{a} Republika}
...
All but the most trivial Agda programs begin by setting some options that effect how Agda behaves and importing from existing libraries, and modules in the \agdaualib are no different. However, all imports in the \agdaualib come exclusively from Martin Escardo's \typetopology library.  Besides being an excellent foundation on which to build, Martin's library is based on a very terse Martin-L\"of type theory, and each addition to this skeleton frame is very deliberate and carefully explained in the Martin's excellent set of notes, \hottufinagda.  The latter describes the \typetopology library in great detail, and is available on \href{https://www.cs.bham.ac.uk/~mhe/HoTT-UF-in-Agda-Lecture-Notes}{Escardo's website}.\footnote{\href{https://www.cs.bham.ac.uk/~mhe/HoTT-UF-in-Agda-Lecture-Notes}{\url{https://www.cs.bham.ac.uk/~mhe/HoTT-UF-in-Agda-Lecture-Notes}}\\
...
To wit, each file in \typetopology (as well as the \agdaualib) begins with the line
\begin{code}
...
\end{itemize}
Since every file in the \agdaualib begins with the same first line, we won't mention the \AgdaKeyword{OPTIONS} directive again.

\subsection{Imports}\label{imports}
\paragraph*{Universes}
The first module of the \agdaualib, called \texttt{prelude}, begins by importing the \universesmodule from the \typetopology library which provides, among other things, an elegant notation for type universes adopted throughout the \agdaualib.
\begin{code}
...
\AgdaKeyword{public}\<%
\end{code}
Following Martin Escardo, the \agdaualib uses capitalized script letters, e.g., \ab 𝓤, \ab 𝓥, \ab 𝓦, \ab 𝓣 to denote universes. The \universesmodule also defines the ̇ operator mapping a universe \ab 𝓤 to \texttt{Set}\ \ab 𝓤, the latter having type \texttt{Set (lsuc}\ \ab 𝓤\texttt{)}.
%% The level \texttt{lzero} is renamed \ab{𝓤₀}, so \ab{𝓤₀}\ ̇ is an alias for \texttt{Set\ lzero}.
We needn't describe the \universesmodule in great detail. Suffice it to highlight the main notational conventions established in Escardo's \typetopology library and adopted throughout the \agdaualib. In particular, \texttt{Set\ (lsuc\ lzero)} is denoted by \texttt{Set}\ \ab 𝓤₀\ ⁺ which is denoted by \ab 𝓤₀\ ⁺\ ̇. Also, will often need a type that lives in the universe that is the least upper bound of two universe, say, \ab 𝓤~ ̇ and \ab 𝓥\ ̇. The universe \ab 𝓤~⊔~\ab 𝓥~ ̇ denotes this least upper bound.

Besides notations for universes, a fair number of other components are imported from \typetopology. A full list of these appears in Appendix~\ref{sec:imports-from-type-topology}.  Hereinafter, we won't mention the imports declared in each \agdaualib module.
...
\AgdaSymbol{)}\<%
\end{code}
Here \AgdaBound{𝓞} represents the universe that operation symbol types inhabit, while arity types inhabit the universe \AgdaBound{𝓥}.
...
Let us emphasize, \algebraUS \emph{is the type inhabited by all algebras of signature} \ab{𝑆} \emph{and carrier type} \ab 𝓤\ ̇, and the type of this type of algebras turns out to be \ab 𝓞 ⊔ \ab 𝓥 ⊔  \ab 𝓤\ ⁺\ ̇. The type \algebraUS doesn't define what an algebra \emph{is}; it merely defines a type of algebras. Certain algebras inhabit this type---namely, the algebras consisting of a universe (say, \AgdaBound{A}) of type \ab 𝓤 ̇, and a collection \AgdaSymbol{(}\AgdaBound{𝑓}\AgdaSpace{}\AgdaSymbol{:}\AgdaSpace{}\AgdaOperator{\AgdaFunction{∣}}\AgdaSpace{}%
...
The \agdaualib defines syntactic sugar that allows us to replace \AgdaOperator{\AgdaFunction{∥}}\AgdaSpace{}\AgdaBound{𝑨}\AgdaSpace{}\AgdaOperator{\AgdaFunction{∥}}\AgdaSpace{}\AgdaBound{𝑓}\AgdaSpace{} with the slightly more intuitive \AgdaBound{𝑓}\AgdaSpace{}\AgdaOperator{\AgdaFunction{̂}}\AgdaSpace{}\AgdaBound{𝑨}.%
...
We make the standard assumption that there is always a sufficiently large collection \AgdaBound{X} of variable symbols at our disposal. For instance, given an algebra \AgdaBound{𝑨} we will want to assume that we can always find a surjective mapping of type \AgdaBound{X}\AgdaSpace{}\AgdaSymbol{→}\AgdaSpace{}\AgdaOperator{\AgdaFunction{∣}}\AgdaSpace{}\AgdaBound{𝑨}\AgdaSpace{}\AgdaOperator{\AgdaFunction{∣}}. We define the type that we will use when making this assumption as follows.
...
Of course, congruences play a central role in mathematics, and this is especially so in universal algebra. 

\subsection{Binary relations}\label{binary-relations-kernels}
Before defining a type for congruences, we first need a type for binary relations.  The standard one from the
\texttt{Binary/Core.agda} module of the \agdastdlib, translated into Escardo's universe notation is
\begin{code}%
...
\AgdaOperator{\AgdaFunction{̇}}\<%
\end{code}
Though the definition that tends to be used more often is the following special case:
...
An example of a binary relation is the ``trivial'' relation. This could be defined in Agda as a general type, or as an inhabitant of the \AgdaFunction{Rel}\AgdaSpace{}\AgdaBound{A}\AgdaSpace{}\AgdaGeneralizable{𝓤} type that we just defined.
\begin{code}
...
Three natural and essentially equivalent ways to define the kernel of a function in type theory are as a general type, as a relation, or as a predicate.
...
For a binary relation ≈ on A, the ≈-class containing 𝑎 is denoted in \agdaualib by \AgdaOperator{\AgdaFunction{[}}\AgdaSpace{}%
\AgdaBound{a}\AgdaSpace{}\AgdaOperator{\AgdaFunction{]}}\AgdaSpace{}\AgdaBound{≈}.
\begin{code}
...
\AgdaBound{x}\<%
\end{code}
In set theory, the quotient of a set 𝐴 modulo an equivalence relation ≈ is properly regarded as the set of ≈-classes of elements from the set.  We define a \textbf{quotient type} to capture this notion. We denote the quotient of a \emph{type} \AgdaBound{A} modulo an equivalence relation type \AgdaBound{≈} by \AgdaBound{A}\AgdaSpace{}\AgdaOperator{\AgdaFunction{//}}\AgdaSpace{}\AgdaBound{≈} and we define this quotienting type as an infix operation.
\begin{code}
\>[0]\AgdaOperator{\AgdaFunction{\AgdaUnderscore{}//\AgdaUnderscore{}}}\AgdaSpace{}%
\AgdaSymbol{:}%
\>[8]\AgdaSymbol{(}\AgdaBound{A}\AgdaSpace{}%
...
It is important to observe that the inhabitants of this quotient type are not mere \AgdaBound{≈}-classes (as we typically view elements of quotients in set theory).  Instead, an inhabitant \AgdaBound{𝒂} of the type \AgdaBound{A}\AgdaSpace{}\AgdaOperator{\AgdaFunction{//}}\AgdaSpace{}\AgdaBound{≈} is by definition a triple, \AgdaSymbol{(}\AgdaSpace{}\AgdaBound{C}\AgdaSpace{},\AgdaSpace{}\AgdaBound{𝑎}\AgdaSpace{},\AgdaSpace{}\AgdaFunction{𝑝}\AgdaSpace{}\AgdaSymbol{)}, where \AgdaBound{C} is a \AgdaBound{≈}-class, \AgdaBound{𝑎} has type \AgdaBound{A}\AgdaSpace{}, and \AgdaFunction{𝑝} is a proof that \AgdaBound{C} is the \AgdaBound{≈}-class containing \AgdaBound{𝑎}; that is, \AgdaBound{C}\AgdaSpace{}%
\AgdaOperator{\AgdaDatatype{≡}}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{[}}\AgdaSpace{}%
\AgdaBound{a}\AgdaSpace{}%
...
Those who have experience with quotients will already appreciate the implication of the above definition.  Since a quotient class always comes equipped with a representative element (as well as a proof that the representative belongs to the class), we never have to worry about choosing representatives. This is important if we want our proofs to be constructive since we will often need to get our hands on quotient class representatives, and we want to do so without appealing to the Axiom of Choice.
\begin{code}\input{congruences/relationClasses.tex}\end{code}\todo{next say something about is-subsingleton-valued}
...
Finally, we come to the definition of congruence. The \agdaualib defines congruence inside a module so that an ambient signature \ab 𝑆 is available.
