...
\AgdaBound{≈} a binary relation on \AgdaBound{A}, and \AgdaBound{a}\AgdaSpace{}\AgdaSymbol{:}\AgdaSpace{}\AgdaBound{A},
then the \AgdaBound{≈}-class containing \AgdaBound{𝑎} is denoted in \agdaualib by
\AgdaOperator{\AgdaFunction{[}}\AgdaSpace{}\AgdaBound{a}\AgdaSpace{}\AgdaOperator{\AgdaFunction{]}}\AgdaSpace{}\AgdaBound{≈}.
\begin{code}
...
As another example, we prove that if 𝑔 is a homomorphism from 𝑨 to 𝑩, if ℎ is a homomorphism from 𝑨 to 𝑪, if ℎ is surjective, and if ker ℎ ⊆ ker 𝑔, then there exists a homomorphism ϕ from 𝑪 to 𝑩 such that 𝑔 = ϕ ∘ ℎ.  This claim is formalized in the \agdaualib as follows:
...
The range or ``image'' of a homomorphism plays a significant role in universal algebra, and the following seem to be (for our purposes) the two most useful types for representing homomomrphic images of an algebra.
...
Here we describe the \termsmodule of the \agdaualib which, after the usual assortment of preliminaries, defines the \AgdaDatatype{Term} datatype that represents the type of terms. The type \ab 𝑋 : \ab 𝒰 ̇ represents an arbitrary collection of variables symbols.\\[-5mm]
\begin{code}\input{terms/term.tex}\end{code}
Using the \AgdaDatatype{Term} type, we now define the term algebra type, denoted here by \AgdaFunction{𝑻}\AgdaSymbol{(}\AgdaBound{X}\AgdaSymbol{)}.
\begin{code}\input{terms/termalg.tex}\end{code}
The universe is the collection of terms over \AgdaBound{X} and the operations are those defined in the \AgdaInductiveConstructor{node} constructor of the \AgdaDatatype{Term} datatype.
...
\AgdaOperator{\AgdaFunction{∣}}\AgdaSpace{}%
\AgdaBound{𝑨}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{∣}} lifts to a homomorphism from \AgdaFunction{𝑻}\AgdaSymbol{(}\AgdaBound{X}\AgdaSymbol{)} to \AgdaBound{𝑨}, and
...
This section describes the \subsmodule of the \agdaualib where subuniverses and subalgebras are defined and some basic facts about them proved.

%% \subsection{Preliminaries}\label{sec:preliminaries}
%% The \subsmodule starts, as usual, by fixing a signature 𝑆 and satisfying some dependencies.

%% \subsection{Types for subuniverses}\label{sec:types-for-subuniverses}
%% \begin{code}\input{subuniverses/subprelude.tex}\end{code}

The module begins with a straightforward definition of the collection of subuniverses of an algebra \algA. Since a subuniverse is a subset of the domain \univA of \algA, it is defined as a predicate on this domain. Thus, a collection of subuniverses will be a predicate on predicates on \univA.
...
Now, once we recall the definition of \AgdaFunction{HomImage} from the \homsmodule, described above in Section~\ref{sec:homomorphic-images},
\begin{code}
...
\subsection{Algebraic Structures}\label{sec:algebraic-structures}
...
\subsubsection{Subuniverse generation}\label{sec:subuniverse-generation}
...
%% \subsection{Subdirect products}\label{sec:subdirect-products}
...
\subsection{Clones}\label{sec:clones}
...
\subsection{Terms and free algebras}\label{sec:terms-and-free-algebras}
...
\subsubsection{Terms}\label{sec:terms}
...
\subsubsection{Free algebras}\label{sec:free-algebras}
...
\subsection{Arity}\label{sec:arity}
...
\begin{comment}
\subsubsection{Essential arity}\label{sec:essential-arity}
...
\subsection{Basic facts}\label{sec:basic-facts}
...
\subsubsection{Equalizers}\label{sec:equalizers}
...
\subsubsection{Homomorphisms}\label{sec:homomorphisms-1}
...
We formalize Obs 14 and its proof in the \birkhoffmodule of the \agdaualib, as described in Section~\ref{sec:itemStyles}. %obs 14 in agda.
...
\subsection{Birkhoff's theorem}\label{sec:birkhoffs-theorem}
...
\subsubsection{Public imports}\label{sec:public-imports}
...
\subsection{Dependent pair type}\label{sec:dependent-pair-type}
...
\subsection{Dependent function type}\label{sec:dependent-function-type}
...
\subsection{Function extensionality}\label{sec:function-extensionality}
...
\subsection{Predicates, Subsets}\label{sec:predicates-subsets}
...
\subsubsection{The membership relation}\label{sec:the-membership-relation}
...
\subsubsection{Subset relations}\label{sec:subset-relations}
...
\subsection{Miscellany}\label{sec:miscellany}
...
\subsection{More extensionality}\label{sec:more-extensionality}
...
\subsection{Unicode Hints}\label{sec:unicode-hints}
