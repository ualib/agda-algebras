...
The type of (indexed) products of a collection of algebras (of the same type) is also a type of algebras which we define as follows:\footnote{In agda2-mode ⨅ is typed as \textbackslash Glb.}
\begin{code}%
...
An example of a binary relation is the ``trivial'' or ``diagonal'' or ``identity'' relation. This could be defined in Agda as a general type, or as an inhabitant of the \AgdaFunction{Rel}\AgdaSpace{}\AgdaBound{A}\AgdaSpace{}\AgdaGeneralizable{𝓤} type that we just defined.
\begin{code}
...
Three natural and essentially equivalent ways to define the kernel of a function in type theory are as a general type, as a relation, or as a predicate.  These are implemented in \agdaualib as follows:
...
Reflexivity, symmetry, and transitivity of a binary relation (say, \texttt{≈}) on \texttt{X}, is defined in the obvious way. Here is reflexivity. (Symmetry and transitivity are similar.)
\begin{code}
...
For a binary relation ≈ on A, the \agdaualib denotes the ≈-class containing 𝑎 by {[} 𝑎 {]} ≈.
\begin{code}
...
\AgdaBound{x}\<%
\end{code}
We denote by \AgdaBound{A}\AgdaSpace{}\AgdaOperator{\AgdaFunction{//}}\AgdaSpace{}\AgdaBound{≈}\AgdaSpace{}
%𝐴 // ≈
the collection of all ≈-classes of 𝐴, defined as follows:
