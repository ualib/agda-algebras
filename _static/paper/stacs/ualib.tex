...
Next we prove
\begin{itemize}
\item \AgdaFunction{pclo-id1}: if every algebra in the class \AB 𝒦 satisfies a particular identity, say 𝑝 ≈ 𝑞, then every algebra in the closure \AgdaDatatype{PClo} of \AB 𝒦 under the taking of arbitrary products also satisfies 𝑝 ≈ 𝑞, and, conversely,
\item \AgdaFunction{pclo-id2}: if every algebra of the product closure \AgdaDatatype{PClo} of \AB 𝒦 satisfies 𝑝 ≈ 𝑞, then so does every algebra in \AB 𝒦.
\end{itemize}
The second item is obvious since \AB 𝒦 ⊆ \AgdaDatatype{PClo}.
\begin{code}\input{closure/pclo-id.tex}\end{code}

\paragraph*{Identities for subalgebra closure}
Here we prove
\begin{itemize}
\item \AgdaFunction{sclo-id1}: if every algebra in the class \AB 𝒦 satisfies 𝑝 ≈ 𝑞, then so does every algebra in the closure \AgdaDatatype{SClo} of \AB 𝒦 under the taking of subalgebras; and, conversely,
\item \AgdaFunction{sclo-id2}: if every algebra of the subalgebra closure \AgdaDatatype{SClo} of \AB 𝒦 satisfies 𝑝 ≈ 𝑞, then so does every algebra in \AB 𝒦.
\end{itemize}
...
We prove
\begin{itemize}
\item \AgdaFunction{hclo-id1}: if every algebra in the class \AB 𝒦 satisfies a 𝑝 ≈ 𝑞, then so does every algebra in the closure \AgdaDatatype{HClo} of \AB 𝒦 under the taking of homomorphic images; and, conversely,
\item \AgdaFunction{hclo-id2}: if every algebra of the homomorphic image closure \AgdaDatatype{HClo} of \AB 𝒦 satisfies 𝑝 ≈ 𝑞, then so does every algebra in \AB 𝒦.
\end{itemize}
...
Finally, we prove
\begin{itemize}
\item \AgdaFunction{vclo-id1}: if every algebra in the class \AB 𝒦 satisfies a 𝑝 ≈ 𝑞, then so does every algebra in the closure \AgdaDatatype{VClo} of \AB 𝒦 under the taking of homomorphic images, subalgebras, and products; and, conversely,
\item \AgdaFunction{vclo-id2}: if every algebra of the varietal closure \AgdaDatatype{VClo} of \AB 𝒦 satisfies 𝑝 ≈ 𝑞, then so does every algebra in \AB 𝒦.
\end{itemize}
