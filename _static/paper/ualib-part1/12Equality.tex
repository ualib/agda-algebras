...
\end{code}
\ccpad
Thus, whenever we need to complete a proof by simply asserting that \ab{x}, or the (possibly implicit) thing in question, is definitionally equal to itself, we can invoke \af{refl} \ab x, or (in the implicit case) \af{refl\ \_} or even \ab{𝓇ℯ𝒻𝓁}. (The \ab{pattern} directive above is what makes last option available.)

Let us now formalize the obvious fact that \ab{≡} is an equivalence relation. 
\ccpad
\begin{code}%
\\[\AgdaEmptyExtraSkip]%
...
\end{code}
\ccpad
The only difference between \af{≡-trans} and \af{≡-TRANS} is that the arguments of \af{≡-TRANS} function are implicit so we can omit them when applying the function.

We use the symmetry of \ab{\_≡\_} very often and we can sometimes improve the readability of a proof by employing some syntactic sugar.\footnote{%
...
A simple but useful operation that we will make heavy use of is sometimes called \textbf{transport} (or ``transport along an identity''). It is defined in the \texttt{MGS-MLTT} module of the \typetopology library as follows.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{0}}]%
