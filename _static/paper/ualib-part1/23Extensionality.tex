...
\subsubsection*{Function extensionality} %\label{sec:defin-funct-extens}
...
For example, the latter is defined as follows.
\ccpad
\begin{code}%
...
% \end{code}
% \ccpad
However, we decided to remove all instances of global function extensionality from the latest version of the \ualib and limit ourselves to local applications of the principle. This has the advantage of making transparent precisely how and where the library depends on function extensionality. The price we pay for this precision is a library that is littered with extensionality postulates.\footnote{Eventually we will probably remove these postulates in favor of an alternative approach to extensionality; e.g., \emph{univalence} and/or Cubical Agda.}
