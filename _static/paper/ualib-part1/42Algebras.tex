...
\end{code}
\ccpad
What makes the \AgdaFunction{Lift-alg} type so useful for resolving type level errors is the nice properties it possesses. Indeed, the \ualib contains formal proofs of the following facts.
\begin{itemize}
\item \af{Lift-alg} is a homomorphism (see \href{https://ualib.gitlab.io/Homomorphisms.Basic.html#exmples-of-homomorphisms}{Homomorphisms.Basic.html}).
\item \af{Lift-alg} is an algebraic invariant; that is, an algebra is isomorphic to its lift (see \href{https://ualib.gitlab.io/Homomorphisms.Isomorphisms.html#lift-is-an-algebraic-invariant}{Homomorphisms.Isomorphisms.html}).
\item \af{Lift-alg} of a subalgebra is again a subalgebra (see \href{https://ualib.gitlab.io/Subalgebras.Subalgebras.html#lifts-of-subalgebras}{Subalgebras.Subalgebras.html}).
\item \af{Lift-alg} preserves identities (see \href{https://ualib.gitlab.io/Varieties.EquationalLogic.html#lift-invariance}{Varieties.EquationalLogic.html}).
\end{itemize}
...
Using the relation |: (defined in \S~\ref{sec:binary-relations}) this implication is expressed compactly as (\ab 𝑓~\af ̂~\ab 𝑨)~\af{|:}~\ab R; whence, the following definition.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{1}}]%
...
\subsubsection{Compatibility of continuous relations*\protect\cref{starred}}\label{compatibility-of-continuous-relations}

Next we define a type that represents \emph{compatibility of a continuous relation} with all operations of an algebra. Fist, we define compatibility with a single operation using \AgdaFunction{cont-compatible-fun} (defined in \S\ref{continuous-relation-types}).
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{0}}]%
...
\end{code}
\ccpad
Whence, \emph{compatibility of a continuous relation with an algebra} is defined as follows.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{1}}]%
