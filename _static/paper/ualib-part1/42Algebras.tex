...
\begin{itemize}
\item \ar{Lift} is a homomorphism.
\item \ar{Lift} is an algebraic invariant; that is, an algebra is isomorphic to its lift.
\item \ar{Lift} of a subalgebra is again a subalgebra.
\item \ar{Lift} preserves identities.
\end{itemize}
...
If \ab{𝑨} is an algebra and \ab{R} a binary relation, then \af{compatible} \ab 𝑨 \ab R will represents the assertion that \ab{R} is \emph{compatible} with all basic operations of \ab{𝑨}. Recall, informally this means for every operation symbol \ab{𝑓} \as : \af ∣ \ab 𝑆 \af ∣ and all pairs \ab x \ab y~\as :~\af{∥} \ab 𝑆 \af{∥} \ab 𝑓 \as → \af ∣ \ab 𝑨 \af ∣ of tuples from the domain of \ab 𝑨, the following implication holds: if \ab{R} (\ab x \ab i) (\ab y \ab i) for all \ab{i}, then \ab{R} ((\ab 𝑓 \af ̂ \ab 𝑨) \ab x) ((\ab 𝑓 \af ̂ \ab 𝑨) \ab y).

The formal definition representing this notion of compatibility is easy to write down since we already have a type, called \af{compatible-fun}, that does all the work.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{1}}]%
...
\end{code}
\ccpad
% In case it helps the reader understand \af{cont-compatible-op}, we redefine it explicitly without the help of
% \af{cont-compatible-fun}. %\href{Algebras.Algebras.html\#fn2}{2}
% \ccpad
% \begin{code}%
% \>[0][@{}l@{\AgdaIndent{0}}]%
% \>[1]\AgdaFunction{cont-compatible-op'}\AgdaSpace{}%
...
% \AgdaSymbol{→}\AgdaSpace{}%
% \AgdaSymbol{(}\AgdaFunction{lift-cont-rel}\AgdaSpace{}%
...
With \af{cont-compatible-op} in hand, it is a trivial matter to define a type that represents \emph{compatibility of a continuous relation with an algebra}.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{0}}]%
