...
\defn{proposition}. We now define a type that captures this concept. (Recall, \af{Pred A 𝓦} is the function type
\ab{A → 𝓦 ̇}.)
\ccpad
\begin{code}%
% \>[0]\AgdaKeyword{module}\AgdaSpace{}%
...
\end{code}
\ccpad
Recall, we defined the relation \af{≐} for predicates as follows: \ab{P} \af{≐} \ab Q \as = (\ab P \af ⊆ \ab Q) \af × (\ab Q \af ⊆ \ab P). Therefore, if we assume \af{prop-ext} \ab 𝓦 holds, then it follows that \ab{P} \aod ≡ \ab Q.
\ccpad
\begin{code}%
\>[0]\AgdaFunction{prop-ext'}\AgdaSpace{}%
\AgdaSymbol{:}%
...
proposition}. As above, we use the \af{is-subsingleton} type of the \typetopology library to impose this truncation assumption on a binary relation.\footnote{%
...
We defined a type called \af{ConRel} in the \ualibhtml{Relations}{Continuous} module to represent relations of arbitrary arity. Naturally, we define a type of \emph{truncated continuous relations}, the inhabitants of which we will call \defn{continuous propositions}.
\ccpad
\begin{code}%
