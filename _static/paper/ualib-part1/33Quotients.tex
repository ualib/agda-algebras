...
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaBound{𝓤}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{̇}}\AgdaSymbol{\}(}\AgdaOperator{\AgdaBound{R}}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{Rel}\AgdaSpace{}%
\AgdaBound{A}\AgdaSpace{}%
...
A prominent example of an equivalence relation is the kernel of any function.
 % We prove that such a kernel is an equivalence relation on the domain of the function as follows.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{1}}]%
...
\subsubsection*{Quotients} %\label{equivalence-classes-1}

If \ab R is an equivalence relation on \ab A, then for each \ab u~\as :~\ab A there is an \defn{equivalence class} (or \defn{equivalence block}, or \ab R-\defn{block}) containing \ab u, which we denote by \af [~\ab u~\af ]~\as :=~\{\ab v~\as :~\ab A~|~\ab R~\ab u~\ab v\}.  We call this the \ab R-\defn{block containing} \ab 𝑎.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{1}}]%
...
\end{code}
\ccpad
Thus, \ab v~\as ∈~\af [~\ab u~\af ] if and only if \ab R~\ab u~\ab v, as desired.  We often refer to \af [~\ab u~\af ] as the \ab R-\defn{block containing} \ab u.
A predicate \ab C over \ab A is an \ab R-block if and only if \ab C~\ad ≡~\af [~\ab u~\af ] for some \ab u~\as :~\ab A.  We represent this characterization of an \ab R-block as follows.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{0}}]%
...
\end{code}
\ccpad
Thus, a proof of the assertion \af{IsBlock} \ab C is a dependent pair (\ab u , \ab p), with \ab u~\as :~\ab A and \ab p is a proof of \ab C~\ad ≡~\af [~\ab u~\af ]\{\ab R\}.

If \ab R is an equivalence relation on \ab A, then the \defn{quotient of} \ab A \textit{modulo} \ab R is denoted by \ab A~\af /~\ab R and is defined to be the collection \{\af [~\ab u~\af ]~∣~\ab y~\as :~\ab A\} of all \ab R-blocks.
\ccpad
\begin{code}%
% \>[0]\<%
% \\
...
\end{code}
\ccpad
It will be convenient to have the following subset inclusion lemmas on hand when
working with quotient types. (Assume a context including
\AgdaBound{𝓤}\AgdaSpace{}%
\AgdaBound{𝓦}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
...
\end{code}
\ccpad
An example application of these is the \af{block-ext} type in the \ualibhtml{Relations.Extensionality} module.
