...
In set theory, given two sets \ab A and \ab B, we say that \ab A is a \defn{subset} of \ab B, and we write \ab A \af ⊆ \ab B, just in case \as ∀~\ab x~(\ab x~\af ∈~\ab A~\as →~\ab x~\af ∈~\ab B). We need a mechanism for representing this notion in Agda. of A typical approach is to use a \emph{predicate} type, denoted by \af{Pred}.

Given a universe \ab 𝓤 and a type \ab A, the type \af{Pred} \ab A \ab 𝓤 can be viewed as representing a property that elements of type \ab A may or may not satisfy.  We write \ab P \as : \af{Pred} \ab A \ab 𝓤 to represent the semantic concept of a collection of elements of type \ab A that satisfy the property \ab P. Here is the definition (which is similar to the one found in the \texttt{Relation/Unary.agda} file of the \agdastdlib.
\ccpad
\begin{code}%
\>[0][@{}l@{\AgdaIndent{0}}]%
...
Below we consider predicates over the class of all algebras of a particular type. Soon (in the \ualibhtml{Algebras.Algebras} module) we will define the type \af{Algebra} \ab 𝓤 \ab 𝑆 of \ab 𝑆-algebras with domain type \ab 𝓤\af ̇, and the type \af{Pred}(\af{Algebra} \ab 𝓤 \ab 𝑆) \ab 𝓦 (whose inhabitants are maps of type \af{Algebra}~\ab 𝓤~\ab 𝑆~\as →~\ab 𝓦\af ̇) will be used to represent subclasses of \ab 𝑆-algebras with certain properties.
