...
What if we had started out this discussion with two functions \(f\) and \(g\) both of which take a list as input and produce as output a correctly sorted version of that list? Are the functions the same? What if \(f\) were defined using the \href{https://en.wikipedia.org/wiki/Merge_sort}{merge sort} algorithm, while \(g\) used \href{https://en.wikipedia.org/wiki/Quicksort}{quick sort}. I think most of us would now agree that \(f\) and \(g\) are not equal in this case.

In the examples above, it is common to say that the two functions \(f\) and \(g\) are \href{https://en.wikipedia.org/wiki/Extensionality}{extensionally equal}, since they produce the same (external) output when given the same input, but they are not \href{https://en.wikipedia.org/wiki/Intension}{intensionally equal}, since their (internal) definitions differ.

In this subsection, we describe types that manifest this idea of \emph{extensional equality of functions}, or \emph{function extensionality}. (Most of these are found in the \typetopology library, so the \ualib merely imports the definitions from there.)

\subsubsection{Definition of function extensionality}\label{sec:defin-funct-extens}

As explained above, a natural notion of function equality, sometimes called \emph{point-wise equality}, is defined as follows: \(f\) and \(g\) are said to be \emph{pointwise equal} provided \(∀ x → f x ≡ g x\). Here is how this notion of equality is expressed as a type in the \typetopology library.
\ccpad
\begin{code}%
...
\end{code}
\ccpad
\defn{Function extensionality} is the assertion that point-wise equal functions are ``definitionally'' equal; that is, \(\forall\ f\ g\ (f ∼ g → f ≡ g)\). In the \typetopology library, the types that represent this notion are \af{funext} (for nondependent functions) and \af{dfunext} (for dependent functions). They are defined as follows.
\ccpad
\begin{code}%
...
Although for some readers it will be obvious, we would like to call attention to the important distinction between type definitions similar in form to \af{funext} and those similar to \af{extfun}.  Notice that the codomain of the former is a generic type, namely, (\ab 𝓤 \as ⊔ \ab 𝓥) \af ⁺ \af ̇ , while the codomain of the latter is the assertion \ab f \af ∼ \ab g. Also, the defining equation of \af{funext} is an assertion, while the defining equation of \af{extdun} is a proof. As such, \af{extfun} is a proof object; it proves (inhabits the type that represents) the proposition asserting that definitionally equivalent functions are point-wise equal. In contrast, \af{funext} is a type that we may or may not wish to assume. That is, we could postulate that function extensionality holds and \emph{assume} we have a witness, say, \ab{fe} \as : \af{funext} \ab 𝓤 \ab 𝓥 (i.e., a proof that point-wise equal functions are equal, but as noted above the existence of such a witness cannot be \emph{proved} in Martin-Löf type theory.
...
First, a type is a \defn{singleton} if it has \emph{exactly} one inhabitant and a \defn{subsingleton} if it has \emph{at most} one inhabitant. These properties are represented in the \typetopology library by the following type.
\ccpad
\begin{code}%
...
\end{code}
\ccpad
Next, we consider the type \af{is-equiv} which is used to assert that a function is an equivalence in a sense that we now describe. First we need the concept of a \href{https://ncatlab.org/nlab/show/fiber}{fiber} of a function. In the \typetopology library, \af{fiber} is defined as a Sigma type whose inhabitants represent inverse images of points in the codomain of the given function.
\ccpad
\begin{code}%
...
extensionality.\footnote{In previous versions of the \ualib we defined the type \af{hfunext} (using another name for it) before realizing that an equivalent type was already defined in the \typetopology library.}
\ccpad
\begin{code}%
