...
We say that a function \ab g \as : \ab A \as → \ab B is \defn{monic} (or \defn{injective}) if it doesn't map distinct elements to a common point. The \af{Monic} type, which we now define, manifests this property. (To reduce reader strain, we omit some easily inferred typing judgments like \ab{A} \as{:} \ab{𝓤}\af ̇ and \ab{B} 
\as{:} \ab{𝓦}\af ̇.)
\ccpad
\begin{code}%
% \>[0]\AgdaKeyword{module}\AgdaSpace{}%
...
\AgdaBound{b}\AgdaSpace{}%
\AgdaOperator{\AgdaInductiveConstructor{,}}\AgdaSpace{}%
\AgdaSymbol{(}\AgdaBound{fafb}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{⁻¹}}\AgdaSymbol{)}\<%
\end{code}
