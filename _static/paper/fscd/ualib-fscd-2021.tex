...
In the \agdaualib, we apply the truncation concept to general relation types rather than to the equality relation on types.  For example, we say that a predicate \af P is a \emph{truncated unary relation} if there is at most one proof that an element satisfies (belongs to) \af P.  Similarly, we call \af P a \emph{truncated binary relation} if there is at most one proof that a pair of elements belongs to \af P or is \af P-related.\footnote{This distinction between standard notions of truncation and ours is merely a change in viewpoint; in fact, it took some time before the distinction was even noticed, and we hesitate to point it out.  On the other hand, the approach is so natural that it is almost surely employed elsewhere. Nonetheless, we describe it here because we have found it useful.}
...
\subsection{Equivalence relations}\label{equivalence-relations}

A binary relation is called a \textbf{preorder} if it is reflexive and
transitive. An \textbf{equivalence relation} is a symmetric preorder.
\ccpad
\begin{code}%
...
\AgdaSymbol{:}%
\>[249I]\AgdaSymbol{\{}\AgdaBound{𝓤}\AgdaSpace{}%
\AgdaBound{𝓦}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaPostulate{Universe}\AgdaSymbol{\}\{}\AgdaBound{A}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
...
If \ab R is an equivalence relation on \ab A, then for each \ab 𝑎 \as : \ab A, there is an \textbf{equivalence class} containing \ab 𝑎, which we denote and define by \af [ \ab 𝑎 \af ] \ab R := all \ab 𝑏 \as : \ab A such that \ab R \ab 𝑎 \ab 𝑏. We often refer to \af [ \ab 𝑎 \af ] \ab R as the \ab R-\emph{class containing} \ab 𝑎.
\ccpad
\begin{code}%
...
\end{code}
\ccpad
So, \ab x \as ∈ \af [ \ab a \af ] \ab R if and only if \ab R \ab a \ab x, as desired.

We define type of all \ab R-classes of the relation \ab{R} as follows.
\ccpad
\begin{code}%
...
\input{Algebras/Congruences.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Homomorphism types %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Homomorphism types}\label{sec:homomorphism-types}
