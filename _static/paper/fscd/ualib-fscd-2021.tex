...
%
Section~\ref{sec:agda-preliminaries} introduces Sigma types in Agda (for us, the most important dependent type in the language) as well as Agda's universe hierarchy. 
%
Section~\ref{sec:types-for-relations} introduces the \agdaualib by explaining how certain relations and quotient types are represented in the library. 
%
Section~\ref{sec:types-for-algebras} describes the type we use to represent algebraic structures.
%
Section~\ref{sec:quotient-types-and-quotient-algebras} describes the approach we take to quotient types and quotient algebra types. 
%
Section~\ref{sec:homom-terms-subalg} defines new types that represent \emph{homomorphisms}, \emph{terms}, and \emph{subalgebras}, and 
%
Section~\ref{sec:equations-and-varieties} does the same for \emph{equational theories} and \emph{models}.
%
...
\end{code}
\ccpad
The \emph{function extensionality type} \af{funext} is then defined by declaring point-wise equal functions equivalent.
\ccpad
\begin{code}
...
An important conceptual distinction exists between type definitions similar in form to \af{funext} and those similar to \af{intesionality}.  Notice that the codomain of the former is a generic type, namely,
\AgdaSymbol{(}\AgdaBound{𝓤}\AgdaSpace{}%
\AgdaOperator{\AgdaPrimitive{⊔}}\AgdaSpace{}%
\AgdaBound{𝓥}\AgdaSymbol{)}\AgdaOperator{\AgdaPrimitive{⁺}}%
...
\AgdaBound{f}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{∼}}\AgdaSpace{}%
\AgdaBound{g}.  Also, the defining equation of \af{funext} is an assertion, while the defining equation of \af{intensionality} is a proof.  As such, \af{intesionality} is a proof object; it proves (inhabits the type that represents) the proposition asserting that definitionally equivalent functions are point-wise equal. In contrast, \af{funext} is a type that we may or may not wish to \emph{assume}.  That is, we may assume we have a witness, say, \ab{fe} \as : \af{funext} \ab 𝓤 \ab 𝓥, that is, a proof that point-wise equal functions are equivalent, but it cannot be \emph{proved} in Martin-L\"of type theory that such a witness exists.


Besides \af{funext}, other dependent and polymorphic notions of extensionality are represented as types and defined in the \ualib and \typetopology libraries, but we omit the definitions (see~\cite[\S2.4]{DeMeo:2021} and~\cite[\S17-18]{MHE}).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Relations %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Relation Types}\label{sec:relations}
...
\subsubsection{Example: kernel types}\label{kernels}

The \defn{kernel} of the function \ab f \as : \ab A \AgdaSymbol{→} \ab B is defined in informal mathematics as the set \{(x , y) ∈ A × A : f x = f y\}. This can be represented in type theory and Agda in a number of different ways, each of which may be useful in certain contexts. For example, we could define the kernel as a Sigma type,
\ccpad
\begin{code}%
...
\end{code}
\ccpad
For example, the \textit{identity relation} (the kernel of an injective function) could be represented as a Sigma type, a (binary) relation, or predicate.
\ccpad
\begin{code}%
...
\section{Truncation}\label{ssec:truncation-and-sets}
...
\subsection{Type truncation}\label{sec:standard-type-truncation}
...
\subsection{Relation truncation}\label{binary-relation-truncation}
In the \agdaualib, we apply the truncation concept to general relation types rather than to the equality relation on types.  For example, we say that a predicate \af P is a \emph{truncated unary relation} if there is at most one proof that an element satisfies (belongs to) \af P.  Similarly, we call \af P a \emph{truncated binary relation} if there is at most one proof that a pair of elements belongs to \af P or is \af P-related.\footnote{The distinction between the standard notion of truncation and the notion described here is merely a change in viewpoint; in fact, didn't immediately notice the distinction and we even hesitated to point it out.  Moreover, the approach is so natural that it is almost surely employed elsewhere. Nonetheless, we present it here because it has been useful in our formalization efforts.}
...
Here we descirbe some of the key components of the \ualibQuotients module of the \agdaualib. For more details, see \url{https://ualib.gitlab.io/UALib.Relations.Quotients.html}.

\subsection{Properties of binary relations}\label{properties-of-binary-relations}

Let \ab{𝓤} \as : \af{Universe} be a universe and \ab A \as : \ab 𝓤\af ̇ a type.
Above we defined types representing binary relations on \ab A. Here we define types for properties of binary relations and types for relations that have such properties. The most important special properties of relations are represented by the types we now define.
\ccpad
\begin{code}%
...
\subsection{Compatibility of operations and relations}\label{compatibility-of-operations-and-relations}

The following definitions and lemmas are useful for asserting and proving facts about \defn{compatibility} of relations and operations.
\ccpad
\begin{code}%
...
\subsection{Congruence relations types}\label{sec:congr-relat-types}
...
\end{code}
\ccpad
\textbf{Unicode Hints}. The \af ╱ symbol is typed in \texttt{agda2-mode} as \texttt{\textbackslash{}-\/-\/-}, plus \texttt{C-f} a number of times.
