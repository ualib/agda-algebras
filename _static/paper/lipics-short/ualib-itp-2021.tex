...
Although the \ualib project was initiated relatively recently (by the author of this paper, with valuable contributions by Hyeyoung Shin and Siva Somayyajula), the part of universal algebra and equational logic that we formalize here already extends well beyond the scope of the prior efforts mentioned above.  In particular, the \ualib now includes the only formal, constructive, machine-checked proof of Birkhoff's variety theorem that we know of. We remark that, with the exception of~\cite{Carlstrom:2008}, all other proofs of Birkhoff's theorem we have seen are informal and not known to be constructive.

%% \subsection{Vision and Goals}\label{vision-and-goals}
The seminal idea for this project was the observation that, on the one hand, a number of fundamental constructs in universal algebra can be defined recursively, and theorems about them proved by structural induction, while, on the other hand, inductive and dependent types make possible very precise formal representations of these recursively defined objects, and often yield very short, elegant and constructive proofs of properties of such objects.  An important feature of such proofs in type theory is that they are total functional programs and, as such, they are computable and composable.  These observations suggest that there is much to be gained from implementing universal algebra in a language, such as Martin-L\"of type theory, that supports dependent and inductive types.

\subsection{Objectives}\label{objectives}
One goal of this project is to express the foundations of universal algebra constructively, in type theory, and to formally verify the foundations using the Agda proof assistant. Thus we aim to codify the edifice upon which our mathematical research stands, and demonstrate that advancements in our field can be expressed in type theory and formally verified in a way that we, and other working mathematicians, can easily understand and verify.  We hope our library inspires and encourages others to formalize and verify their own mathematical research so that we may more easily understand and verify their results.

As our field is both deep and broad, an endeavor to codify all of its foundations may seem like a daunting and risky investment of time and resources.  However, we believe the subject is well served by a modern, \emph{constructive} presentation of its foundations. Indeed, the mere act of reinterpreting the foundations in an alternative system offers a fresh perspective, which we feel certain will lead us to deeper insights and new discoveries.

We emphasize that our objective is not merely to translate existing results into a modern, formal language, but rather to provide a system that is useful for conducting research into new mathematics.  That is how we intend to use the library now that we have achieved our initial goal of implementing the foundations of the subject in Agda and succeeded in formalizing, as a proof-of-concept, the Birkhoff HSP theorem.

Our own research experience has taught us that a proof assistant, equipped with specialized libraries for universal algebra and domain-specific tactics to automate proof idioms of our field, would be an extremely valuable and powerful tool.  Therefore, our goal is to construct a library that will serve as an indispensable part of the algebra researcher's tool kit.
...
This section describes some aspects of the \href{}{UALib.Prelude.Preliminaries} module of the \agdaualib. That module imports everything we will need from Martin Escardo's Type Topology library. Also many basic types are defined and some of their properties proved.  We do not go over the entire Preliminaries module, but instead focus on those aspects that might differ from standard Agda syntax or cause confusion in the sequel.

\input{Preliminaries-short.tex}

\subsection{Equality}\label{sec:equality}
\input{Equality-short.tex}
...
\input{Extensionality-short.tex}
