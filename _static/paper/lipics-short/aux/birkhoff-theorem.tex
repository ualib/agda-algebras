...
\end{code}
\ccpad
Some readers might worry that we haven't quite acheived our goal because what we just proved (birkhoff) is not an ``if and only if'' assertion. Those fears are quickly put to rest by noting that the converse---that every equational class is closed under HSP---was already proved in the \href{UALib.Varieties.Preservation.html}{Equation Preservation} module. Indeed, there we proved the following identity preservation lemmas:
\begin{itemize}
\item
