.. code-block:: lean
    ...
    -- BEGIN
    -- To prove that the free algebra is absolutely free, we
    -- show that the lift of an arbitrary function h : X → A
    -- is a homomorphism and that it is the unique homomorphism 
    -- extending h.
  
    -- Definition of the lift of a function.
    -- Given an arbitrary map h : X → A, define a function on 
    -- terms as follows:
    ...
    -- if the given term is ``app f ts``, then 
    -- ``h (app f ts) = (A f) lift_of h``
