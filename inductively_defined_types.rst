...
Moreover, for the demonstrations in this section, we start with the following "preamble" of excerpts from other parts of the `lean-ualib`_:
...
(In the `lean-ualib`_, these definitions would be imported at the top of the `clone.lean`_ file.)

First, we define a structure that specifies the properties that a clone must have.  In :numref:`clones` we explained that a collection of operations is called a **clone** iff it includes all projections and is closed under general composition.

.. include:: _static/clone.lean.1.rst

Next we define an inductive type that represents the clone generated by a given collection ``𝒪`` of operations.
