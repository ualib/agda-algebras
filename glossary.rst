...
.. glossary::
   ...
   Nick's class
     ...
     :term:`Cobham's thesis` asserts that the class :term:`P` can be thought of as the class of "tractable" problems; similarly, NC can be thought of as the problems that are tractable using parallelism.    
   ...
   polynomial time
    ...
    :term:`Cobham's thesis` asserts that the class P can be thought of as the class of "tractable" problems.
 
   tractable CSP

     Combining the main results of :cite:`Berman:2010` and :cite:`Idziak:2010` yields the following theorem. 

     **Theorem**. Let 𝐀 be a finite idempotent algebra with an edge term. Then 𝐀 is tractable. 

     Tractability is largely preserved by familiar algebraic constructions.  For example, (:cite:`Bulatov:2005`) if 𝐀 is a finite, idempotent, tractable algebra, then every subalgebra and finite power of 𝐀 is tractable, and every homomorphic image of 𝐀 is :term:`locally tractable`.  
...
.. glossary::
   ...
   power
     The **power** (or **cardinality**) of an L-:term:`theory` T is denoted by |T| and defined to be the cardinality of the language L.
...
.. _structures-categories-varieties:
...
.. glossary::
   ...
   absorbing
     ...
     We denote the fact that 𝐁 is an absorbing subalgebra of 𝐀 with respect to some term by writing :math:`𝐁 ◁ 𝐀`. If we wish to be explicit about the term, we write :math:`𝐁 ◁_t 𝐀`.
   ...
   edge term
     ...
     Clearly every edge term is :term:`idempotent <idempotent term>` and a :term:`Taylor term`; also, every :term:`Malcev term` and every :term:`near unanimity term` is an edge term.
  ...
  left module
   ...
   Malcev product
     Let 𝒱 and 𝒲 be idempotent varieties. The **Malcev product** of 𝒱 and 𝒲 is the class
     ...
     Freese and McKenzie show in :cite:`Freese:2016` that a number of important properties are preserved under Malcev product; in particular, they prove,
   ...
   Taylor variety
     Walter Taylor proved in :cite:`Taylor:1977` that a variety 𝒱 satisfies some nontrivial idempotent Malcev condition if and only if it satisfies one of the following form: for some :math:`n`, 𝒱 has an idempotent :math:`n`-ary term  :math:`t` such that for each :math:`i∈ \{1, \dots, 2\}` there is an identity of the form
...
.. glossary::
   ...
   neighborhood
     A **neighborhood** of a point :math:`p` in a :term:`topological space` :math:`X` is a set :math:`A` in :math:`X` whose :term:`interior` contains :math:`p`; that is, :math:`p ∈ A^o`.
...
#. PCSP(3-color,100-color) (or even just PCSP(3-color,6-color)) is not known to be NP-complete :cite:`Barto:2018a`, :cite:`Brakensiek:2016`
...
#. Let ℓ be the list of properties preserved under :term:`Malcev product`, and suppose 𝒱 has one property in ℓ and 𝒲 has another.  Is every finite member of 𝖧(𝒱 ∘ 𝒲) is tractable?
