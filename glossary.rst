...
.. glossary::
    ...
    eval
      The **eval** (or **function application**) operation is denoted by :math:`\mathbf{eval}: (A → B) × A` and defined as follows: for each :math:`f: A → B` and :math:`a: A`, we have :math:`\mathbf{eval} (f, a) = f a`.
      
      For example, if :math:`h: ∏_{a:A}(C_a → D)` and :math:`k: ∏_{a:A} C_a` and :math:`a:A`, then :math:`\mathbf{eval} ∘ \mathbf{fork}(h)(k)(a) = h(a)k(a): D`.
