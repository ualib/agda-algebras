...
+  **`Classical.Magma` — pattern-setting first concrete classical structure** (M3-3, #331).  Introduces the magma signature ([`src/Classical/Signatures/Magma.lagda.md`](src/Classical/Signatures/Magma.lagda.md)), the Σ-typed core that degenerates to a type alias for the empty-theory base case `Magma α ρ = Algebra Sig-Magma α ρ` ([`src/Classical/Structures/Magma.lagda.md`](src/Classical/Structures/Magma.lagda.md)), the bidirectional bundle bridge to `Algebra.Bundles.Magma` with a pointwise round-trip lemma ([`src/Classical/Bundles/Magma.lagda.md`](src/Classical/Bundles/Magma.lagda.md)), the level-fixed `lzero`–`lzero` veneer ([`src/Classical/Small/Structures/Magma.lagda.md`](src/Classical/Small/Structures/Magma.lagda.md)), and `(ℕ, +)` as the canonical first worked example ([`src/Examples/Classical/Magma.lagda.md`](src/Examples/Classical/Magma.lagda.md)).  The module-header prose of [`Classical.Structures.Magma`](src/Classical/Structures/Magma.lagda.md) is *normative* for every subsequent classical structure's signature mechanics: hyphen-separated `<symbol>-Op` constructor naming, `ar-<Structure>` arity-function naming, `Sig-<Structure>` signature-value naming, ASCII `^` for operation-symbol interpretation, the curried-user-facing-accessor pattern `_∙_ = Curry₂ (∙-Op ^ _)`, and the `fromOp`-family constructor pattern.  Subsequent structures (Semigroup in M3-4, Monoid and Group in M3-6, Lattice in M3-7, Ring in M3-8) consume this template; deviations require their own ADR.  See [ADR-002 v2 §1, §5, §6, §7, §8](docs/adr/002-classical-layer-design.md) for the rationale.
