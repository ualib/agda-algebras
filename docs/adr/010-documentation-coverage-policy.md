...
+  **One definition per fence is explicitly rejected** as a repository-wide rule.  It enforces layout rather than content — 1969 paragraphs each restating a type signature would satisfy it — and it destroys the narrative reading that makes the library usable as an exposition of universal algebra.
...
| measure | value |
|---|---|
| public definitions, in 307 live modules | 3247 |
| definitions whose fence carries no real prose (the enforced bar) | 201 |
| definitions failing the rejected one-per-fence bar | 1969 |
...
+  **One definition per fence, repository-wide** (the literal reading of the style guide).  Rejected because it enforces layout rather than content, costs 1969 blocks against 201, and destroys the narrative reading.  It is not rejected as a *local* choice: `Setoid.Algebras.Basic` ([#538][]) meets it, and modules that are genuinely reference material may do the same.
