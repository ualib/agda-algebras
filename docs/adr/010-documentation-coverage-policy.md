...
+  **One definition per fence is rejected** as a repository-wide rule — decided, not merely recommended.  It enforces layout rather than content — 1952 paragraphs each restating a type signature would satisfy it — and it destroys the narrative reading that makes the library usable as an exposition of universal algebra.
...
+  **The rejected bar.**  One definition per fence is not required, and is not adopted.  A fence may introduce a family of related definitions under a single block, and should when the definitions are genuinely a family.  `--strict` remains in the tool as a *diagnostic* — a way to see how much prose a module would need under the rejected reading — never as a gate.
...
+  **Neutral.**  `docs/STYLE_GUIDE.md` § "Every public definition has a prose comment block" said "immediately above it", which read as the rejected bar.  It is reconciled with this ADR in the same change: the rule attaches to the fence, one definition per fence is not required, and a block covering several definitions should name each.  There is now one statement of the bar, not two.
