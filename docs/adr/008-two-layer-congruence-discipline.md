...
At the same time, the library's finiteness interfaces — the bare carrier record `FiniteAlgebra` of `Setoid.Algebras.Finite` and the congruence-side `FiniteCongruences` of `Setoid.Congruences.Finite`, split along exactly this seam by #465 — provide decidable congruences (`DecCon`) with decidable pair-membership, and isolate the classical content of "finite" in the `complete` field of `FiniteCongruences`.  A decision was needed on which object the FLRP formalization quantifies over, and how the classical content is accounted for, before work packages WP-3 through WP-6 proceed.
...
+  **Layer D (decidable)**.  Finitely presented congruences with decidable membership on finite finitary algebras; reconstruction of every decidable congruence from its list of related pairs; a constructive completeness theorem for the decidable congruences (interface `FiniteCongruencesᵈ`, the constructive counterpart of `FiniteCongruences`), obtained by enumerating Bool-valued tables; and `Representableᵈ`, over the `DecCon` poset, as the FLRP program's working notion.  The lemma stack (L1–L5) and audits are specified in `docs/notes/flrp-two-layer-congruences.md`.
+  **The bridge**.  The `complete` field of `FiniteCongruences` — every semantic congruence is `≑`-equal to a decidable one — is the single classical assumption of the program, registered in the planned `FLRP.Assumptions` with its strength documented (between weak excluded middle and excluded middle at the working level).
