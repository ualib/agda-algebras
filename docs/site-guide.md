...
+  **`docs/_links.md`** — a library of reference-style links (`[Setoid.Algebras.Basic][]`,
   `[ADR-008][]`) appended to every page so cross-references resolve.  Its module
   and ADR sections are generated by `scripts/python/gen_links.py`; re-run that
   (`make gen-links`) after adding a module or an ADR.  CI runs it with `--check`
   and rejects any drift, so the file cannot fall behind the tree.
...
links* section of `docs/_links.md`.  For a module or an ADR you need not touch
the file by hand — `make gen-links` regenerates those sections from `src/` and
`docs/adr/`.

These references are the site's recurring failure mode: an undefined `[label][]`
does not error, it renders as literal text and sails past `mkdocs build
--strict`.  Two pure-Python guards, bundled as `make check-links` and run in CI
(the *Docs cross-links* job), close that gap:

+  `gen_links.py --check` fails if `docs/_links.md` has drifted from the `src/`
   and `docs/adr/` trees — so a new module or ADR cannot ship without its link
   definition;
+  `check_links.py` scans the rendered corpus and fails if any reference-style
   link is undefined — catching typos and non-module targets the generator
   cannot know about.  It skips the frozen `src/Legacy/` prose (rebrand tracked
   separately), generated `GITHUB_PROJECT.md`, and code spans.
