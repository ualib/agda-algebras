...
level of finite alternating chains and — modulo one isolated, clearly-finitary hypothesis
— at the level of the library's actual congruence join.  The forward Day direction is
*deferred*, because its staircase is genuinely not a mechanical mirror of Jónsson's.  Both
asymmetries are explained below.

## What landed (all in `Setoid.Varieties.MaltsevConditions`)

+  `Chain 𝑩 R` — the **finite alternating chain** relation: the reflexive–transitive
   closure `x ≈ · R · R ⋯ R · ≈ y` of a relation `R` on the carrier of `𝑩`.  Used with
   `R = φ ∪ᵣ ψ`, so each `cons` step is tagged (by the `⊎` of `_∪ᵣ_`) as a φ-step or a
   ψ-step.  The carrier algebra is an *explicit* parameter, since it cannot be inferred
   from a relation on `𝕌[ 𝑩 ]` (the carrier projection is not injective).  `Chain⊆Gen`
   records that a chain is below the generated join `Cg(φ ∪ ψ)` (`base`/`rfl`/`tran`).
...
+  The honest scope is therefore: prove everything against `Chain` (`jonsson⇒chainDistributive`,
   ...
   whole development.  `JoinIsChain` holds for finitary signatures, where `Gen` adds
   nothing beyond the transitive closure of the union; discharging it (a one-coordinate-
   at-a-time argument that needs a finite-arity enumeration, then `Cg-least`) is a
   self-contained successor task, deliberately not rushed here.
...
+  **The obstruction is a feature, named once.**  Rather than silently assume finitary
   arities, the development proves the general chain statement and surfaces `JoinIsChain`
   as the lone finitary lever — keeping the theorem honest about exactly what the
   infinitary `comp` costs.
...
+  The changed module: `nix develop --command agda src/Setoid/Varieties/MaltsevConditions.lagda.md`.
