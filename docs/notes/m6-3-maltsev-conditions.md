...
is deferred rather than rushed.  (All three are now wired — after the M6-7 cleanup, in
`Setoid.Varieties.FreeSubstitution` (`subhom`, `cg-pair→⊢`),
`Setoid.Homomorphisms.Properties` (`Cg⊆ker`), and `Setoid.Varieties.SoundAndComplete`
(`toEq`); see [`m6-4-free-bridge.md`](m6-4-free-bridge.md).)  (The issue text says "the free algebra on two
