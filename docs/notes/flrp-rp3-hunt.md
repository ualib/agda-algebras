...
+  **Socle-level enforcement entries** (the F10 gap, filed as [issue #560](https://github.com/ualib/agda-algebras/issues/560)): obtain the Aschbacher `D∆`/signalizer primary texts and work the specialization to parachutes, per RP-2's § 5 and § 6; any resulting entry immediately reopens the candidate table.  Its formal prerequisite, a simplicity notion with a certified `A₅` (needed even to state monolith-refined classes), landed with [issue #562](https://github.com/ualib/agda-algebras/issues/562): `Classical.Structures.Group.Simple` and `Examples.Classical.Groups.AlternatingGroup5`.
