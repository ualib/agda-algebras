...
**Update (M6-18, #510)**.  Descent is now a theorem — `Classical.Structures.Group.MinimalNormalDescent`, and `finite-MinimalNormalDescent` of `FLRP.Reductions` discharges this very property — but the antecedent does not disappear, and the reason is worth recording because it was not anticipated when the decision above was taken.  `MinimalNormalDescent` demands minimality against *every* normal subgroup, with nontriviality in the negative form `¬ (N ⊆ 1)`, and in that reading it is **classical**: `minimal→DNE` shows an unrestricted-minimal normal subgroup of a finite group decides `¬ ¬ P → P` for every proposition at the working level, by an oracle subgroup `M ∩ (1 ∪ P)` in the style of the WP-1 oracle congruence.  What is unconditional is the *witnessed* form, whose minimality clause still ranges over all normal subgroups and restricts only the shape of their nontriviality hypothesis.  So the antecedent survives, but changes character: it is now `DecidablyPresented` — the group-side reading of `complete` of `FiniteCongruences`, the library's one Layer-S bridge — rather than an unproved statement of finite group theory.  See `docs/notes/m6-18-minimal-normal-descent.md` § 3.
