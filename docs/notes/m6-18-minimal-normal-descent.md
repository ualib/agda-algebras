...
+  **A worked instance.**  `𝟏-FiniteAlgebra` exists, but the descent is vacuous on the one-element group.  A small nonabelian example — the smallest for which the minimal normal subgroup is not the whole group — would exercise `⟪_⟫` computationally and is the natural regression test.  It is not needed for the theorem and would cost a concrete group construction the `Classical/` tree does not yet have.
