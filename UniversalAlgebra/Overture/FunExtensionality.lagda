...
module hide-∼ {A : Type 𝓤 } where

 _∼_ : {B : A → Type 𝓦 } → Π B → Π B → Type (𝓤 ⊔ 𝓦)
 f ∼ g = Π x ꞉ A , f x ≡ g x
...
module hide-funext where
 ...
 dfunext : ∀ 𝓤 𝓦 → Type (lsuc (𝓤 ⊔ 𝓦))
 dfunext 𝓤 𝓦 = {A : Type 𝓤 }{B : A → Type 𝓦 }{f g : Π x ꞉ A , B x} →  f ∼ g  →  f ≡ g
...
Before moving on to the next subsection, let us pause to make a public import of the original definitions of the above types from the [Type Topology][] library so they're available through the remainder of the [UniversalAlgebra][] library.
...
happly : {A : Type 𝓤 }{B : A → Type 𝓦 }{f g : Π B} → f ≡ g → f ∼ g
happly refl _ = refl

\end{code}


Though it may seem obvious to some readers, we wish to emphasize the important conceptual distinction between two different forms of type definitions by comparing the definitions of `dfunext` and `happly`. In the definition of `dfunext`, the codomain is a parameterized type, namely, `Type (lsuc (𝓤 ⊔ 𝓥))`, and the right-hand side of the defining equation of `dfunext` is an assertion (which may or may not hold). In the definition of `happly`, the codomain is an assertion, namely, `f ∼ g`, and the right-hand side of the defining equation is a proof of this assertion. As such, `happly` defines a *proof object*; it *proves* (or *inhabits the type that represents*) the proposition asserting that definitionally equivalent functions are pointwise equal. In contrast, `dfunext` is a type, and we may or may not wish to postulate an inhabitant of this type. That is, we could postulate that function extensionality holds by assuming we have a witness, say, `fe : dfunext 𝓤 𝓥`, but as noted above the existence of such a witness cannot be proved in [MLTT][].
...
module hide-hfunext where
 ...
 hfunext 𝓤 𝓦 = {A : Type 𝓤}{B : A → Type 𝓦} (f g : Π B) → is-equiv (happly{f = f}{g})
