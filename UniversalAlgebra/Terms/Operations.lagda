...
\begin{code}
 ...
 free-lift-interp : dfunext 𝓥 𝓤 → (𝑨 : Algebra 𝓤 𝑆){X : Type 𝓧 }(η : X → ∣ 𝑨 ∣)(p : Term X)
...
\begin{code}
 ...
 term-gen : dfunext 𝓥 (ov 𝓧) → {X : Type 𝓧}(p : ∣ 𝑻 X ∣) → Σ q ꞉ ∣ 𝑻 X ∣ , p ≡ (𝑻 X ⟦ q ⟧) ℊ
 term-gen _ (ℊ x) = (ℊ x) , refl
 term-gen fe (node 𝑓 𝑡) = node 𝑓 (λ i → ∣ term-gen fe (𝑡 i) ∣) , term-interp 𝑓 (fe λ i → ∥ term-gen fe (𝑡 i) ∥)


 term-gen-agreement : (fe : dfunext 𝓥 (ov 𝓧)){X : Type 𝓧}(p : ∣ 𝑻 X ∣) → (𝑻 X ⟦ p ⟧) ℊ ≡ (𝑻 X ⟦ ∣ term-gen fe p ∣ ⟧) ℊ
 term-gen-agreement _ (ℊ x) = refl
 term-gen-agreement fe {X} (node f 𝑡) = ap (f ̂ 𝑻 X) (fe λ x → term-gen-agreement fe (𝑡 x))

 term-agreement : dfunext 𝓥 (ov 𝓧) → {X : Type 𝓧}(p : ∣ 𝑻 X ∣) → p ≡  (𝑻 X ⟦ p ⟧) ℊ
 term-agreement fvx p = snd (term-gen fvx p) ∙ (term-gen-agreement fvx p)⁻¹
...
\begin{code}
 ...
 --module _ {𝓤 𝓦 𝓧 : Level}{X : Type 𝓧} where
...
\begin{code}
 ...
 module _ {𝓦 𝓤 : Level}{X : Type 𝓤} where
