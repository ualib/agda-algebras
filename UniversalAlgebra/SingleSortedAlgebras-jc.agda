...
⟦ C ⟧s ξ .Carrier = ⟦ C ⟧ (Carrier ξ)

⟦ Op ▷ Ar ⟧s ξ ._≈_ (op , args) (op' , args') = Σ[ eq ∈ op ≡ op' ] EqArgs eq args args'
 where
 EqArgs : (eq : op ≡ op') → (Ar op → ξ .Carrier) → (Ar op' → ξ .Carrier) → Set _
 EqArgs refl args args' = (i : Ar op) → ξ ._≈_ (args i) (args' i)

⟦ Op ▷ Ar ⟧s ξ .isEquivalence .IsEquivalence.refl                        = refl , λ i → Setoid.refl ξ
⟦ Op ▷ Ar ⟧s ξ .isEquivalence .IsEquivalence.sym (refl , g)              = refl , λ i → Setoid.sym ξ (g i)
⟦ Op ▷ Ar ⟧s ξ .isEquivalence .IsEquivalence.trans (refl , g) (refl , h) = refl , λ i → Setoid.trans ξ (g i) (h i)
