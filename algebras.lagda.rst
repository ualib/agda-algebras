...
Let 𝒦 be a class of algebras and 𝑼 = ⟨𝑈, …⟩ an algebra of the same type as the members of 𝒦. Let 𝑋 be a subset of 𝑈. We say that

  #. 𝑼 has the :term:`universal mapping property` for 𝒦 over 𝑋 if for every 𝑨 ∈ 𝒦 and every function ℎ : 𝑋 → 𝐴, there is a homomorphism ℎ̃ : 𝑼 → 𝑨 that agrees with ℎ on 𝑋;
...
The next definition is used in order to take a free algebra *for* a class 𝒦 and obtain a free algebra *in* 𝒦.
...
Notice that it is possible for :math:`ψ(𝒦)(𝑨)` to be empty, in which case :math:`Ψ(𝒦)(𝑨) = 1_A` and then :math:`𝑨/Ψ(𝒦)(𝑨)` is obviously trivial.

We now apply these ideas to the algebra 𝑻(𝑋) of 𝑆-terms over 𝑋. Since 𝑻(𝑋) is free for 𝒜𝓁ℊ, it follows that for any subclass 𝒦 of 𝒜𝓁ℊ, 𝑻(𝑋) is free for 𝒦 over 𝑋, although it is not necessarily a member of 𝒦. Taking the quotient modulo :math:`Ψ(𝒦)(𝑻(𝑋))`, we obtain an algebra free *in* SP(𝒦).
...
Alternatively and equivalently we could define "equational class" and "equational theory" in terms of the two :term:`closure operators <closure operator>` induced by the Galois pair (Mod, Th).  Indeed, Mod Th : 𝒫(𝒜) → 𝒫(𝒜) is a closure operator on 𝒜 and Th Mod : 𝒫(ℰ) → 𝒫(ℰ) is a closure operator on ℰ, and 
...
.. proof:theorem::
   ...
   .. container:: toggle
      ...
      Let 𝒲 be a class of algebras closed under 𝑯, 𝑺, and 𝑷 (i.e., a variety). We must find a set Σ of equations that axiomatizes 𝒲, as this will prove that 𝒲 is the class of algebras satisfying the set of equations Σ (i.e., that 𝒲 is an equational class). The obvious choice for Σ is to use the set of all equations that hold in 𝒲. That is, we take Σ = Th(𝒲). Let 𝒲' = Mod(Σ).  Clearly, 𝒲 ⊆ 𝒲'. We prove the reverse inclusion.

      Let 𝑨 ∈ 𝒲' and 𝑌 a set of cardinality max(∣𝐴∣, ω). Choose a surjection ℎ₀ : 𝑌 → 𝐴. By :numref:`Obs %s <obs 9>`, ℎ₀ extends to an epimorphism ℎ : 𝔉(𝑌) → 𝑨`. Furthermore, since :math:`𝔽_𝒲(Y) = 𝔉(Y)/Θ_𝒲`, there is an epimorphism :math:`g: 𝔉(Y) → 𝔽_𝒲`. We claim that :math:`\ker g ⊆ \ker h`. If the claim is true, then by :numref:`Obs %s <obs 5>` there is a map 𝑓 : 𝔽_𝒲(𝑌) → 𝐴 such that :math:`f ∘ g = h`. Since ℎ is epic, so is 𝑓. Hence :math:`𝑨 ∈ 𝑯(𝔽_{𝒲}(Y)) ⊆ 𝒲` completing the proof.

      Now we prove the claim. Let :math:`u, v ∈ 𝔉(Y)` and assume :math:`g(u) = g(v)`. Since 𝔉(Y) is generated by 𝑌, there are terms 𝑝, 𝑞 ∈ 𝔉(Y) and 𝒚 such that :math:`u = p^𝔉(𝒚)` and :math:`v = q^𝔉(𝒚)`. Applying the homomorphism :math:`g`,

      .. math:: p^{𝔽_𝒲} 𝒚 = g(u) = g(v) = q^{𝔽_𝒲} 𝒚.

      Thus 𝒲 ⊧ 𝑝 ≈ 𝑞, hence (𝑝, 𝑞) ∈ Σ. Since 𝑨 ∈ Mod(Σ) we get 𝑨 ⊧ 𝑝 ≈ 𝑞. Therefore, :math:`ℎ(𝑢) = 𝑝^𝑨(ℎ₀ ∘ 𝒚) = 𝑞^𝑨(ℎ₀ ∘ 𝒚) = ℎ(𝑣)`, as desired.
