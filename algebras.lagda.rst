...
One goal of our project is to provide, as a "proof of concept" a formal implementation of a deep result in universal algebra. As the focus of this goal, we have chosen what was among the first major results of the theory of universal algebras---namely, the celebrated `HSP Theorem`_ that `Garrett Birkhoff proved in 1933 <https://gitlab.com/ualib/ualib.gitlab.io/-/blob/master/_static/lit/Birkhoff-OnTheStructureOfAbstractAlgebras.pdf>`_. (The original paper is available `here <https://gitlab.com/ualib/ualib.gitlab.io/-/blob/master/_static/lit/Birkhoff-OnTheStructureOfAbstractAlgebras.pdf>`_.)
...
It should be clear that 𝔽(𝒦, 𝑋) is a :term:`subdirect product` of the algebras \{𝑻(𝑋)/θ\}, where θ ranges over ψ(𝒦, 𝑻(𝑋))\}, so it belongs to SP(𝒦). Therefore, we say that 𝔽(𝒦, 𝑋) is free *in* SP(𝒦).

We formalize the free algebra in :numref:`the free algebra in agda`.
...
.. proof:theorem:: Thm 4.41 of :cite:`Bergman:2012`
   ...
   .. container:: toggle
      ...
      Let 𝒲 be a class of algebras closed under H, S, and P (i.e., a variety). We must find a set ℰ of equations that axiomatizes 𝒲 (that is 𝒲 = Mod ℰ), as this will prove that 𝒲 is the class of algebras satisfying the set of equations ℰ and thus establish that 𝒲 is an equational class.

      The obvious choice for ℰ is Th 𝒲, the set of all equations that hold in 𝒲.

      Let 𝒲† = Mod ℰ.  Clearly, 𝒲 ⊆ 𝒲†, so we must prove the reverse inclusion.

      Let 𝑨 ∈ 𝒲† and let 𝑋 be a set of cardinality max(∣𝐴∣, ω).

      Choose a surjection ℎ₀ : 𝑋 → 𝐴. By :numref:`Obs %s <obs 9>`, ℎ₀ extends to an epimorphism ℎ : 𝑻(𝑋) → 𝑨.

      Since 𝔽(𝒲, 𝑋) = 𝑻(𝑋)/Ψ(𝒲, 𝑋), there is an epimorphism ε : 𝑻(𝑋) → 𝔽(𝒲, 𝑋).

      If ker ε ⊆ ker ℎ, then by :numref:`Obs %s <obs 5>` there would be a map 𝑓 : 𝔽(𝒲, 𝑋) → 𝐴 such that 𝑓 ∘ ε = ℎ; moreover, 𝑓 would be epic (since ℎ is) and this would give 𝑨 ∈ H(𝔽(𝒲, 𝑋)) ⊆ 𝒲, as desired.

      So it suffices to prove that ker ε ⊆ ker ℎ.
      ...
      Therefore, 𝒲 ⊧ 𝑝 ≈ 𝑞, so (𝑝, 𝑞) ∈ ℰ and 𝑨 ⊧ 𝑝 ≈ 𝑞, since 𝑨 ∈ Mod ℰ.

      Combining these facts yields the desired conclusion that (𝑢, 𝑣) belongs to ker ℎ; viz.,
