...
and the resulting algebra is free *in* SP(𝒦).
...
.. proof:theorem:: Thm. 4.38 of :cite:`Bergman:2012`
   ...
   .. container:: toggle
      ...
      Recall that :math:`𝔽(𝒦)(X) = 𝑻(X)/λ(𝒦)(𝑋) ∈ 𝑺𝑷(𝒦)`. We show (1) ⟹ (3) ⟹ (2) ⟹ (1).
      ...
      (2) ⟹ (1). We wish to apply :numref:`Obs %s <obs 14>`. Let 𝑨 ∈ 𝒦 and :math:`h ∈ \mathrm{Hom}(𝑻(𝑋), 𝑨)`. Then :math:`𝑻(𝑋)/\mathrm{ker} h ∈ 𝑺(𝑨) ⊆ 𝑺(𝒦)` so :math:`\mathrm{ker} h ⊇ λ(𝒦)(𝑋)`.  Thus, (2) implies :math:`h p = h q` hence (1) holds.
...
.. proof:observation:: Cor. 4.39 of :cite:`Bergman:2012`
   ...
   .. container:: toggle
      ...
      Since :math:`𝔽(𝒦)(Y) ∈ 𝑺 𝑷(𝒦)`, the left-to-right direction uses the same argument as in :numref:`Thm %s <Thm 4.38>`.  So assume that :math:`p^{𝔽(𝒦)(Y)}(y₁, …, yₙ) = q^{𝔽(𝒦)(Y)}(y₁, …, yₙ)`. To show that 𝒦 ⊧ 𝑝 ≈ 𝑞, let 𝑨 = ⟨𝐴, 𝑓^𝑨 ⟩ ∈ 𝒦 and 𝑎₁, ..., 𝑎ₙ ∈ 𝐴. We must show :math:`p^𝑨(a₁, …, aₙ) = q^𝑨(a₁, …,aₙ)`. There is a homomorphism :math:`h : 𝔽_𝒦(Y) → (A, f^𝑨)` such that :math:`h(yᵢ) = aᵢ` for :math:`i ≤ n`. Then,

      .. math:: p^𝑨(a₁, …, aₙ) = p^𝑨(h(y₁), …, h(yₙ)) = h(p^{𝔽(𝒦)(Y)}(y₁, …,yₙ)) = h(q^{𝔽(𝒦)(Y)}(y₁, …,yₙ)) = q^𝑨(h(y₁), …, h(yₙ)) = q^𝑨(a₁, …, aₙ).
