...
If 𝑇(𝑋) is nonempty, then we can impose upon it an algebraic structure, which we denote by 𝔉. We call 𝔉 the **term algebra in the signature** 𝑆 **over** 𝑋; it is constructed as follows:
  ...
  For every basic operation symbol 𝑓 ∈ 𝐹 let :math:`f^𝔉` be the operation on 𝑇(𝑋) that maps each tuple 𝑠 : ρ𝑓 → 𝑇(𝑋) to the formal term 𝑓 𝑠; define 𝔉 to be the algebra with universe 𝑇(𝑋) and basic operations :math:`\{f^𝔉 | f ∈ F\}`.
